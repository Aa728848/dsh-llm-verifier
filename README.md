...
方向是可靠的（A/B 换槽后完全一致），但**效果量随难度塌缩**：explicit-tag 一次只采一个字母，量化下限就是 1/19 ≈ 0.053，0.018 的中位差意味着 3 条判据里只有 1 条动了一档。**所以「让插件起草」只在预期候选之间有明显质量差时才划算**（不同思路、不同实现正是这种情形）；如果候选很可能都「做了真实工作」，先花钱买判官质量（换成能走概率期望通道的判官）而不是把 N 调大。设置页的「判官自检」会回报当前实际通道。
