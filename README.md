...
typecheck 门禁按 `package.json` 的 `devDependencies` 解析，已随宿主推进到 `0.1.7-rc.2`（此前从 `0.1.1-rc.2` 上移到 `0.1.7-alpha.1`）：客户端面的图标名在 0.1.7 才改名，一份源码只能对齐一侧，宿主面的旧形态兼容由运行时代码与回归测试保证。从 `0.1.7-alpha.1` 到 `0.1.7-rc.1` 的宿主改动对本插件消费的所有宿主面**全是加法**（`tools/pre-execute` 侧只新增可选 `projectContent`，`dsh-session` / `dsh-llm` / `dsh-settings` / `dsh-agent` / `dsh-credentials` / 客户端槽位与图标包源码零改动），因此这一步只换版本线，不动插件代码。

`0.1.7-rc.2` 同样只换版本线：这一版给宿主加了「动态工具更新」机制，对插件全是新增的可选面（`dsh-llm` 的 `GenerateOptions.toolHistory?` 与 `ToolUpdate`、`dsh-session` 的 `Session.toolHistory()`、`dsh-tools` 的 `PreToolDecision.ask.displayReason?`）。插件不消费这套机制，宿主新增的事件与块类型也读不到它：宿主在工具名变化时追加的 `developer/message`（`role: 'developer'`，内容为 `tool-addition` / `tool-removal` 块）不属于观测证据，插件的 `if/else` 分发自然跳过它们；验收的"过期"判定只统计有结果的工具调用与 PTC 派发，因此这类中途追加的消息不会让已完成的验收失效。另需注意 DeepSeek provider 在这一版裂成两个包（`llm-deepseek-api-key` 与 `llm-deepseek-account`），但 profile 里的**条目 id 仍是 `llm-deepseek`**，设置命名空间与其中的 `baseURL` / `apiKeyEnv` 都没变，直连 logprobs 通道照常读到；新增的 `deepseek-account` provider 走账号令牌、没有 API key，直连通道对它不适用，按设计降级到显式标签通道。
...
| **每任务/每会话最多路由** | 限制自动路由**周期**的次数：`compare/select/track`、语义分类、团队任务闸、计划预审与 P06 的过程选优周期（过程周期同样消耗本项额度，因此它受既有路由额度约束；另有独立的过程周期每任务上限——恢复触发档 1 次，每步选优档 `maxProcessCyclesPerTask` 次）。**一次周期只消耗一次**——语义分类成立时会在同一个预约上直接提升为它解析出的 `compare/select/track`（分类＋执行＝1 次），因此默认 2 次足以完成「计划预审 → 分类＋compare」；解析为 `none`、低置信、非法引用，或分类成功但执行预算不足时同样消费该周期（后者记录为 `classification-only-budget`，不会伪装成"未路由"或"已评审"）。它与最终验收**各自独立计数**（路由用本项，最终验收用「每任务/每会话最多验收」，互不占用），因此被路由要求过的最终验收一定有额度可用；旧版共享一个计数器时，路由可以把最终验收的额度花光，导致 turn 在从未验收的情况下静默关闭。预算按真实对局数估算（8 候选约需 108 次调用）；被拒绝时写入宿主日志并区分「预算不足 / 已有 verifier 在跑 / 同一证据已路由」，strict 下额外注入一次提示。每个周期还会在统计里留下一条 `route` 观测：周期 id、触发点、去向、预约调用数、证据保留/省略数与取消标记——它是诊断口径，**不是**模型调用数。分类去重按**渲染后的证据（提示词）**而非最后事件序号，追加纯叙述不会再次付费分类；周期 id 取 epoch＋实例＋序号，跨插件重载唯一，重载前后两个不同周期不会在看板与离线汇总里合并；没有预约的诊断行（预算丢弃、交付阶段跳过、证据读取失败 evidence-unreadable）共用同一套 epoch 命名空间 |
| **同时验收子 Agent** | 默认关闭；开启后 `subagent` / fork 子会话也会执行自动路由与最终验收 |
| **接入真实文件改动证据** | 默认**开启**。会话验收（显式 `verifier_current_session` 与自动最终验收）把**宿主自己记录的**本轮文件改动作为证据交给裁判：改动文件清单（路径 + 增删行数）与逐文件前后对比（`@@` 块 + 带 `+`/`-`/空格前缀的行），经参考上下文分隔块进入提示词（因此改动不同必然重新评分，评分缓存键包含该文本）。它修的是「裁判只能从 `tool/call` 的参数反推 Agent 声称改了什么」——改完没落盘、或事后又被改写，过去与真实改动无从区分。限长沿用同一套预算：整块不超过「单项字符上限」（截断提示计入），最多渲染 8 个文件（超出在表头注明），其余字符按文件数用 `itemBudget` 分摊、逐文件按行截断并注明省略行数；`binary` / `oversized` 文件只列一行说明、不读取对比。宿主未提供该服务（0.1.1/0.1.5）、会话已销毁、`summary` 或 `diff` 报错时一律**静默降级为不带该证据**并在宿主日志告警，绝不阻断验收。需要 DSH 0.1.6+ 宿主 |
| **过程选优** | 三档（旧布尔值兼容：`true`→`recovery`，`false`/缺省→`off`）：`off`（默认）完全不进入该路径（不生成候选、不调用裁判、不缓冲响应）；`recovery`（恢复触发）仅当同一任务最近两次已完成的验证运行**都失败**（「失败」从运行自己的判决读：`[exit code: N≠0]`、`N failed`、`error TS…`、`test result: FAILED`、`FAIL` 都算，工具级报错也算；`0 failed` 不算失败）时，为**下一次主模型请求**额外生成备选回复，每任务最多 1 个周期；`every-step`（每步选优）为**每个主循环请求**登记选优意图，无需失败信号，每任务最多 `maxProcessCyclesPerTask` 个周期。三档均**仅 smart 模式**生效（manual/strict 不进入该路径），触发后机制相同：额外生成备选（默认 N=2：原回复＝候选 0，新增一份＝候选 1；`autoProcessCandidates` 可提到 3 或 4，那时按锦标赛判定；采样温度不低于生成档位，避免低温会话买到原回复的副本），按 **process 判据**（失败靶向 / 与已失败尝试不同 / 可验证的一步）比较后**只回放胜者**。过程周期有自己的每任务上限（独立于路由与最终验收的第三本账），同时也消耗「每任务/每会话最多路由」额度，并受任务/会话模型调用预算与最终验收保留额度共同限制；生成与比较共用一个 `timeoutMs` 阶段截止时间。裁判视图里的任务、约束、最近失败证据与两份候选（含工具名与参数）**先脱敏再限长**，并按实际渲染文本计量总量，工具动作无法完整容纳时回退原回复。选优**不是**验收：胜者交给宿主后，最终验收照常覆盖随后产生的实际工作 |
| **备选带上失败证据** | 默认**开启**，仅在过程选优非 `off` 时生效。`recovery` 档把触发本周期的那两次失败验证运行（先 `sanitizeVerifierText` 脱敏，再按单项/总量双层限长，总量 4000 字符）作为一条插件 user 消息附在备选请求之后，让额外生成的候选是针对真实失败的一次不同尝试，而不是同一提示词的再抽样；`every-step` 档只在任务恰好出现两连败信号时附带，无信号则不带；关闭即对照组（两份候选掌握的信息完全相同，周期照买）。裁判**不被告知**哪份候选带了证据，避免按来源而不是按内容打分；统计行的 `route.alternativeAugmented` 记录该差异，供真实任务对照读取。 |
| **备选用另一个模型** | 默认留空：备选由当前会话模型再采样。可填**逗号分隔**的 `provider/model` 列表（例如 `deepseek-official/deepseek-v4-flash, openai/gpt-5`）：第 i 份备选使用第 i 项，列表短于备选数时循环取用，候选之间因此是真正的不同假设；跨 provider 时**不继承**原请求的推理强度（适配器可能不认识该 id），`maxTokens` 与生成温度下限照旧，归一化后的整个列表记进 `route.alternativeModel`。代价是比较同时也在比「哪个模型更好」，与任务本身的质量不可分——所以它是实验臂，不是推荐配置。 |
| **每任务选优周期上限** | 仅 `every-step` 档生效：单个任务最多购买的过程选优周期数（1–32，默认 4）。它是独立于路由额度与最终验收额度的第三本账；但每个过程周期仍消耗一次「每任务/每会话最多路由」额度（会话默认上限 8），因此每步选优的实际步数上限取两者较小者。周期计数同时保存在路由内存计数与话题侧车 `verifier/process-selection-v1.json`（取两者较大值），插件重载后不失忆 |
| **过程选优候选数** | 默认 **2**（原回复 + 1 份备选，判一对）。设为 3 或 4 时按 `[原回复, 备选 1, …]` 跑锦标赛，判官调用随之上升（N=3 约 5 对，约为单对的 5 倍），预算从同一份任务/会话模型调用额度里出——预约用引擎自己的 `estimateRoutedCalls()` 估算对局数，不会和实际跑的对数脱节。完全相同的候选不会送给判官：引擎先去重，全部相同则 0 调用并回放原回复。**不采纳**上游「多数票直接返回未评判候选」的语义。 |
...
| **输入 / 输出价格** | 手填的 USD/百万 token 单价。**留 0 即交给自动定价**；一旦填了任意一个，整组就以手填为准 |
| **缓存读取价格** | 缓存命中输入 token 的单价；留 0 时退回按输入价计算（旧口径，会把缓存密集的调用算贵）。真实的 cache-read 单价常是输入价的 1/50 |
| **本机模型目录自动定价** | 默认开。本机安装的 pi-ai 模型目录（models.dev 快照，`@earendil-works/pi-ai/dist/providers/data`，约 1350 条）里有这条 provider+模型时自动取单价，**纯离线、零配置** |
| **在线价格库兜底** | 默认开。本机目录查不到时向 models.dev 查一次（4.6 MB 快照，进程内缓存 24 小时），手填或目录命中时**完全不联网** |
| **价格来源 Provider** | 转售路由（例如 command-code）两张表都没有时的显式出路：填一个 provider id（例如 openrouter），按它的挂牌价折算。**留空＝不猜**，未定价路由的成本记为 0 |
...
- 费用估算把生成 token 按**会话模型自己的单价**（同样走手填 > 本机目录 > 在线兜底）折算，缓存读取 token 走缓存单价，因此比只按判官单价折算更接近真实账单；但它始终是估算，不是账单。
...
**计划模式（plan mode）激活期间，自动门控整体挂起**：停止边界的所有自动路由、Team 任务验收与最终验收、`agent/pre-step` 的候选早评审与过程选优登记全部跳过（以会话日志中最后一条 `plan/mode` 事件折叠判定，空日志视为未激活）。此时 Agent 的职责是调研与提案，唯一仍运行的门是 `exit_plan_mode` 计划预审——否则验收必然对着尚未实施的方案判低分，并 steering「去实际执行」，等于在用户批准计划之前就命令 Agent 动手。计划获批退出计划模式后，下一停止边界照常恢复验收。
...
原生主请求 ──► 是否开启且有有效意图？
                ...
                └是 ──► 下游派发一次并缓冲原回复
                         ...
                         └─ 完整 ──► 预约一个 process 周期 → 生成 N-1 份备选（附失败证据）→ process 判据比较/锦标赛
                                      ├─ 备选胜出 ──► 只回放备选
                                      └─ 原胜 / 并列 / 相同 / 失败 ──► 回放原回复
```
...
- **三档开关**（`autoProcessSelection`，旧布尔值兼容：`true`→`recovery`）：`off` 默认关闭，完全不进入该路径（不生成候选、不调用裁判、不缓冲响应）；`recovery` 恢复触发，保持首发语义；`every-step` 每步选优，为每个主循环请求登记意图。三档都只在 smart 模式生效；manual / strict 不进入该路径。
- **默认 N=2（可配 2–4）；每任务周期上限随档位**：`recovery` 每任务最多 1 个周期，`every-step` 每任务最多 `maxProcessCyclesPerTask` 个（1–32，默认 4，路由内存计数与侧车记录取较大值，重载不失忆）。过程周期与现有任务/会话**路由额度**、任务/会话模型调用预算、最终验收保留额度共用（预算不够时在发出任何新增请求之前回退原路径）：一个过程周期在它自己的每任务计数器之外再消耗一次路由额度，因此 `every-step` 的实际步数上限是 `maxProcessCyclesPerTask` 与剩余路由会话额度（默认 8）的较小者；同一会话的第二个任务可以购买自己的周期。
- **触发场景按档位**：`recovery` 的唯一触发场景是同一任务中**最近两次已完成的验证运行都失败**；`every-step` 无需失败信号，每个主循环请求都可触发（有失败信号且开关开启时备选仍附带失败证据）。失败链的判定规则不变——「失败」读的是**运行自己的判决**，而不只是工具状态：`tool/result` / PTC 结果本身报错算失败，宿主的非零退出码写成文本（`[exit code: 1]`、`N failed`、`error TS…`、`test result: FAILED`、`FAIL`）同样算失败。宿主明确对非零退出**不**置 error（`tool-pwsh` 的渲染器原话是 reported, not errored），所以旧实现要求工具级错误时，两次失败的测试运行看起来像两次成功，P06 在真实会话里根本触发不了；`0 failed` 不算失败。两次中只要有一次成功，失败链就断开；识别不出就不触发，也不会为确认触发额外买一次分类调用。
...
**并发派发（投机执行）**：意图在请求派发**之前**就已登记，所以「这次请求可能要选优」在进入 `llm/stream` 时就是已知的。因此购买与备选生成在**原回复还在流式输出时就并发开始**（预约与购买记录仍然先于第一次新增模型调用完成），只有比较仍是串行：新增等待从「原回复 + 生成 + 比较」降到「max(原回复, 生成) + 比较」。代价是**稀有分支要付一次生成**——原回复超限（1 MiB）、finish 不合法、内容为空，或周期在流式中途失效时，旧实现可以不买就跳过，现在会买到一次生成再作废，并如实记成 `purchased: true` + `generatedCalls: 1` 的行（这是真实发生的花费）。阶段截止时间 `timeoutMs` 现在覆盖整个重叠窗口，因此极慢的原回复可能吃掉备选的时间预算。

**已知代价**：输出仍要等原回复完成、并等一次比较（生成与它重叠）才可见；这部分延迟与调用成本必须由真实任务对照评估，不能由钩子存在推导出来。

**聊天提示（这段时间里界面为什么静止）**：周期在跑的时候主回复被整条缓冲，聊天界面**不会有任何流式文字**，看板上也还没落行。宿主那种行内提示（「正在压缩上下文…」）走的是**会话事件 + 客户端节点**，这条路对外部插件是关闭的：持久化读路径（`@deepseek-ai/dsh-session-persistence` 的 `validateStoredEvents`）会拒绝解释含 `KNOWN_SESSION_EVENT_TYPES`（仓库内生成）之外事件类型的日志，除非该记录带 `ignorable: true`，而 `Session.append()` 无法写入这个标记——只有 seed/restore 的持久化信封能带。**所以本插件一条会话事件都不写**，改用内存态回答：选择器按会话登记「本次周期在哪个阶段（生成 / 比较）、几份候选」，插件既有的 RPC 增加一个只读的 `{ kind: 'process', sessionId }`（纯内存查询：不读侧车、不调模型、不改任何判定），聊天输入框上方的 dock 只在**回合运行中、或刚结束的 20 秒内**轮询它，依次显示「正在过程选优：生成 N 份候选…」→「裁判比较 N 份候选…」→「已改用更优的回复 / 保留原回复 / 备选与原回复相同 / 未完成」。它**不进模型历史、不写会话日志、不参与任何判定**；重载后什么都不显示（那个周期确实已经不存在）。长期记录仍然只有统计看板。

同一块指示器也覆盖**自动路由与最终验收**：它们跑在 `agent/turn-stopping` 里，回复已经在屏幕上，但回合会一直开着等裁判（`compare` / `select` / `track` 各 6 次调用起，`select` 随候选数与对数上升；最终验收是判据数 × 轮次 × 裁判数）。发布点是**路由器的预约状态机**——那是唯一知道「一个周期被批准了、什么时候结算」的地方（`RouterCycleObserver`，可选且异常安全：一个坏掉的观察者绝不能改变预约、预算或判决），P06 自己的周期则由选择器发布更细的相位。于是这些窗口会显示「正在识别需要独立复核的对象…」→「正在比较候选方案 / 为多个候选排名 / 复核任务进度（约 N 次调用）…」，最终验收结束时给一次「最终验收通过 / 未通过」——**通过时宿主本来一句话都不说，这一行是唯一能看见它的地方**。这一行是**输入框上方的 dock 卡片**，几何与颜色都跟宿主同层的 Todo / Goal / Queue 卡片同族（同一条内容宽度轴：聊天内容列减去左右留白与四个 dock 内缩，高 36px、圆角 12px、底色 `--dsw-specific-tip`），状态点用宿主的 `StateDot`（进行中 / 完成 / 失败），容器带 `role="status"`，所以「通过」对读屏同样可达；它不是一条横贯窗口的状态行。**这张卡片只有一个元素**：宽度轴与表面（描边、圆角、底色）声明在同一个盒子上——composer stack 是宿主区域，宿主或皮肤样式表可以给 dock 行刷底，多一层「宽而无底」的 wrapper 会让那层底在卡片两侧露出一条遮罩（见 `.agents/notes/implemented/bug-fix/2026-09-15-single-element-activity-card.md`），`client.test.ts` 的 `client activity card structure` 锁死了这一点。计划预审与 Team 任务闸**不进**这块指示器：前者本来就有宿主的挂起工具行，后者窗口极短，加进去只是噪音。
...
- **看板与设置页只用宿主真实存在的主题 token**：所有颜色都写成 `--dsw-alias-*` / `--dsw-specific-*` / `--dsw-static-*` 里的名字，浅色与深色由宿主 token 表解析，不读主题标记、不手写两套色。原因见 `.agents/notes/implemented/bug-fix/2026-09-14-client-theme-tokens-and-activity-chip-geometry.md`：CSS 对取不到值的 `var()` **静默丢弃声明**，所以写错 token 名既不会被构建、类型检查也不会被单测发现，历史上就因此让看板在浅色主题下画出一片深色卡片。`src/client.test.ts` 里的 `client design tokens` 用例现在会双向校验这份名单（用了未登记的名字、或登记了没用的名字，都会失败）。
...
- **自动定价的三层来源**（`src/pricing.ts`）：手填单价 > 本机 pi-ai 模型目录 > models.dev（可选）。宿主**不向插件暴露任何价格**——`@deepseek-ai/dsh-llm` 的 usage 与 `resolveModel` 没有价格字段，`llm-pi-ai` 还明确把 pi-ai 的 cost 元数据归零（"no consumer reports spend"）——所以价格只能由插件自己找。匹配**只认精确的 provider + model**：同一个 model id 在 models.dev 上有 21 家 provider 挂牌、价差 6.5 倍（0.10/0.40 ↔ 0.65/1.45），跨 provider 取价等于往成本列写一个编造的数字，插件因此**绝不猜**；转售路由要用 `priceProviderOverride` 明确指定跟随谁的挂牌价，否则老老实实记为未定价（0）。两层目录都是「读不到就退到下一层」，任何一种失败都不会影响判定。判官自检会回报当前判官实际生效的单价与来源。
...
- **读取宿主设置的两代接缝**：DSH 0.1.1–0.1.6 用 `settings.get(ns)` 读命名空间；0.1.7 把注册式接缝换成了 `SettingsForms`，**它根本没有 `get`**，唯一的公开读取是 `describe()` 的描述符 `value`。插件按能力探测两代接缝，任一代读不到都只意味着不启用直连通道；
