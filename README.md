...
- **运行统计与监控**：每次调用返回完整的统计信息，涵盖实际请求次数、重试次数（Retries）、各类 Token 消耗（输入/缓存命中/输出/推理 Token）、缓存命中与未命中次数（Cache Hits/Misses）以及费用估算；统计记录还会保存一条 `verdict` 摘要（阶段、结果、分数、基线、胜负、阈值；`track` 额外带 `scores` 检查点曲线，`score` 取**最新**检查点而不是历史最低分，旧记录缺少该字段时按旧样式渲染），看板的「最近调用」因此能直接显示每次判决与阈值的关系，回答"这次为什么没通过"，而不是只显示"调用成功/失败"（注意：`success` 表示**模型调用没有抛错**，不等于任务通过验收）；其中：
