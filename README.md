...
| **保存决策快照** | 默认开启：把每次裁判调用的**脱敏提示词 + 原始回答**限量写入本话题的 `verifier/decisions-v1.json`（一次调用 ≤ 12 次模型调用、单条 ≤ 3 万字符、每话题保留最近 40 条），统计看板的「最近调用」里可逐条展开查看。它只回答"裁判为什么这么判"，不参与任何判定；命中评分缓存或复用 in-flight 调用时不会伪造快照 |
