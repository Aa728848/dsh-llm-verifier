...
| **路由评估轮次** | 自动路由每个标准的重复轮次；默认 1。`compare` 只判一对，会被**向上取整到偶数**（引擎只在奇数轮交换 A/B 位置，奇数轮等于让第一个候选固定坐在 A 位）；`select` 的 ring 天然对称、pivot 轮的 A/B 槽位由引擎逐对平衡，`track` 没有位置可换，两者都保留配置值——不对症的偏差不付双倍调用 |
