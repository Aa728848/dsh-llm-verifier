...
同一块指示器也覆盖**自动路由与最终验收**：它们跑在 `agent/turn-stopping` 里，回复已经在屏幕上，但回合会一直开着等裁判（`compare` / `select` / `track` 各 6 次调用起，`select` 随候选数与对数上升；最终验收是判据数 × 轮次 × 裁判数）。发布点是**路由器的预约状态机**——那是唯一知道「一个周期被批准了、什么时候结算」的地方（`RouterCycleObserver`，可选且异常安全：一个坏掉的观察者绝不能改变预约、预算或判决），P06 自己的周期则由选择器发布更细的相位。于是这些窗口会显示「正在识别需要独立复核的对象…」→「正在比较候选方案 / 为多个候选排名 / 复核任务进度（约 N 次调用）…」，最终验收结束时给一次「最终验收通过 / 未通过」——**通过时宿主本来一句话都不说，这一行是唯一能看见它的地方**。计划预审与 Team 任务闸**不进**这块指示器：前者本来就有宿主的挂起工具行，后者窗口极短，加进去只是噪音。
