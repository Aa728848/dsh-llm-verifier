...
| **过程选优（受控）** | 默认**关闭**。开启后**仅 smart 模式**生效，且仅当同一任务最近两次已完成的验证运行**都失败**（真实 tool/result 的错误状态 + 验证输出形状）时，为**下一次主模型请求**额外生成一份备选回复（固定 N=2：原回复＝候选 0，新增一份＝候选 1），按 `proposal` 判据比较后**只回放胜者**。每任务最多 1 个周期，受任务/会话模型调用预算与最终验收保留额度共同限制；生成与比较共用一个 `timeoutMs` 阶段截止时间。选优**不是**验收：胜者交给宿主后，最终验收照常覆盖随后产生的实际工作。关闭时完全不进入该路径（不生成候选、不调用裁判、不缓冲响应）；manual/strict 不进入该路径 |
...
- **固定 N=2、每任务最多 1 个周期**，且与现有任务/会话模型调用预算、最终验收保留额度共用（预算不够时在发出任何新增请求之前回退原路径）。
