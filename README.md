...
```text
task ─┬─ 生成 N 份候选（会话模型，temperature 1.0，maxTokens 4096/份，并行）
      ├─ verifier_select 的锦标赛（默认 3 判据 × 2 轮）→ 相对最优 + 排名
      └─ 胜者 vs 空工作基线（与最终验收同一条基线）→ 绝对分 + passesThreshold
```
...
两条路径的前提都是裁判具备**区分两个都做了真实工作的候选**的能力。若判官在难分候选上退化为极值（A/T），best-of-N 等于付 N 倍生成成本换一次抛硬币——因此先把「判官自检」与难分候选的对比跑一遍再决定是否采用。
