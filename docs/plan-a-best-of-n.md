...
- **Step 3-b（n=3、真实长任务）**：重启宿主后**复验通过**。同一个「函数 + 12 条测试用例 + 一句说明」的任务：`generated: 3`、`failed: 0`、**`truncated: []`**（16384 足够，4096 的失败不再出现）、`calls: 15`（`n=3, repeats=1`：3 生成 + 9 锦标赛 + 3 基线，与公式一致）、`judges[0].calls: 12`（合并基线后 = 9 + 3）。决策快照里三份草稿是**三种不同实现**（手写索引扫描 + 溢出守卫 / `Record<Unit, number>` 查表 / 先散文后代码且空白判断用 `trim().length`），`baseline: ` 前缀也如实落到快照里。
- **真机暴露的一个诚实结论（选优 vs 够好）**：这次三份都合格，锦标赛判成 **0.5 / 0.5 / 0.5 全平**——正是 §5.3 实测的 0.018 难分 gap 在真实任务上的样子，排名没有信息量。真正有用的信息全部来自那次**绝对分**比较：`score: 0.667`、`passesThreshold: false`、`failedCriteria: ['output_match']`——生成侧无法提供「真实运行过的 stdout/stderr」，所以「Output Match」这一条恒为 0，工具据此拒绝放行。**这是「基线比较不可省」最直接的一次实证**：如果没有那次比较，agent 只会拿到一个平局的排名并发货。
- **尚未覆盖**：仍在 explicit-tag 通道下的难分 gap 复测（换判官才有意义）、n=4 的真机成本抽样、以及 `repeats=2` 下的 n=3 真机计数（公式已由单测与 n=2 真机锁定）。
