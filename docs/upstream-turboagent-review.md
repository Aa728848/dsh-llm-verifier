...
### 5.3 决策（基于判别探针实测数据，2026-09-13）：做 A，不做 B

两个 pair 各 10 次、A/B 槽位各 5 次，共 60 次真实裁判调用：

| pair | 难度 | 方向正确率 | 中位 gap | Bradley–Terry 单次胜率 |
|---|---|---|---|---|
| 1 | 明显质量差（一方漏验需求） | 10/10 | 0.544 | 63.3% |
| 2 | 难分（都通过任务列出的用例，差别在 `.strip()` 的覆盖范围与证据） | 10/10 | 0.018 | 50.4% |

- **方向可靠、无位置偏差**（换槽后结果完全一致）；但**效果量随难度塌缩**，难分时单次比较约等于抛硬币。
- 60 次全走 explicit-tag，且这是**结构性**的：`antigravity` 是原生 adapter，未在 `llm-pi-ai.providers` 声明 `api: openai-completions`，而直连 logprobs 的路由只认这种 profile 且强制 HTTPS。explicit-tag 一次采样一个字母，量化下限 1/19 ≈ 0.053，0.018 的中位差说明 3 条判据里只有 1 条动了一档。
- 也顺带更正一条早期结论：曾经看到的"78/78 条比较全是 1.0/0.0"是**与固定空工作基线比较**的产物，不是判官饱和；真实候选之间标尺用得起来（0.44–0.98 都出现过）。

**结论**

- **A（生成侧选优、显式触发）值得做，且已实施**：它作用在"候选差异明显"的场景（不同思路、不同实现），正是探针证明可判的区间，成本也只由愿意付的调用者承担。第一阶段（配方文档 + `verifier_select` 措辞）与第二阶段的 `verifier_best_of_n` 工具都已上线；**上述 0.018 的难分 gap 已作为该工具的使用门槛写进 README**（`docs/plan-a-best-of-n.md` §9 记录了这条 Step 0 回退分支被无条件采用）。
- **B（`llm/stream` 每次调用拦截）不做**：它把 N 倍生成 + 锦标赛的代价摊到**每一次**模型调用上，而绝大多数调用的候选落在"难分"区间。按本仓库实测口径估算：一次被拦截的调用（约 25k 输入）在 3 候选 + 3 判据下约 +2 次生成 + 18 次裁判调用 ≈ **+45 万输入 token**；一个 10 次模型调用的 turn ≈ 450 万，超过本话题整个会话的裁判开销（1.67M）。再叠加 chunk 重放、流式体验、会话日志完整性三处工程风险，以及"从独立复核变成生成层"的定位改变——收益覆盖不了成本。
- **改口条件（可 falsify）**：把判官换成能走概率期望通道的配置后重跑难分探针，若中位 gap ≥ 0.15（单次胜率 ≥ 54%），再重新评估 B；达不到就维持不做。
- **A 的实施计划**见 [`docs/plan-a-best-of-n.md`](plan-a-best-of-n.md)（工具 `verifier_best_of_n`、零新配置项、成本表、验收与风险）；**已实施完毕**，落地偏差与真实成本见该文 §9。
...
### 8.2 交付前自查发现的缺陷（均已修复 + 回归测试）

| 缺陷 | 影响 | 修复 |
|---|---|---|
| `summarizeVerdict` 读 `criteria[].scoreA`，而 `verifySession` 发的是 `{id,name,score}` | **11/11 条会话验收的逐项判据从未落盘**；看板与判决卡都看不到"哪一条没达标"（门控本身未受影响，它直接读结果对象） | `statistics.ts` 同时接受两种形状；单测改用 `verifySession` 的真实形状（原来的测试喂了 compare 形状，所以绿着丢数据） |
| 回放工具把所有快照都当 `<score_A>` 解析 | `track`（`<c1>..<cN>`，且存的是最后一个检查点、方向相反）被误报 `unreadable`——检测回归的工具自己制造假回归 | `replay.ts` 按内容选标签，新增 `not-scored` 类别；真实数据 18/18 match、0 drift |

### 8.3 宿主面接缝测试（本轮补上）

单元测试只证明模块正确，不证明宿主看到的接口正确。补了两处装配级验证：

- **设置 schema 往返**：`Config → JSON → Config → resolveConfig`，覆盖全部 6 个预设值、trim 行为与默认值（宿主就是用这个 schema 校验已存配置的）；
- **判官自检路由**：经插件装配驱动——无 agent 时必须说明原因、默认用 coding 判据、自定义文件不可用时**带原因**退回 coding、真实文件能解析、判官失败必须**被报告**而不是崩溃或静默成功。
