...
- **A（生成侧选优、显式触发）值得做**：它作用在"候选差异明显"的场景（不同思路、不同实现），正是探针证明可判的区间，成本也只由愿意付的调用者承担。已上线的"配方文档 + `verifier_select` 措辞"就是 A 的第一阶段；`verifier_best_of_n` 工具等通道问题修好、难分 gap 重测之后再决定。
