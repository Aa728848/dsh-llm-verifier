...
- **失败行必须保留失败前已成功的用量**：`engine.ts` 在 job/repeat 的 worker 里累计用量、并在"全失败"抛错前把共享 `RunStats` 挂到错误上（`partialStats()` 读取），`select` 的 `scorePairs` 同样按 pair 累计；`index.ts` 的失败分支用它而不是只信 `requestAttempts()`（partial 已含失败请求自身的 attempts，不再重复相加）并标记 `usageIncomplete`。**别再回到"所有 job 完成后才汇总"**——那会把 2 次成功 + 1 次失败记成 1 次调用、0 token。
