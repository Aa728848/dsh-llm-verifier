...
- **读取宿主设置命名空间没有 `get` 了**：0.1.1–0.1.6 用 `ctx.get('settings').get(ns)` 读 `llm-deepseek` / `llm-pi-ai` 的配置；0.1.7 把注册式接缝换成 `SettingsForms`，其原型上**只有 `configure` / `describe`**，调 `settings.get(ns)` 会抛 `settings.get is not a function` 并让整次验收崩掉。`top-logprobs.ts` 的 `settingsNamespace()` 是这一差异的**唯一**读取点：先探测 `get`，否则走 `describe()` 里 `ns === ns` 那条描述符的 `value`（`describe()` 需要 Active fiber，读不到就当作没有直连通道）。新增读取点**不要**再直接写 `settings.get(...)`。
- **0.1.7-rc.2 的「动态工具更新」与 provider 拆包是有意不动的两处**：(a) 宿主把工具声明的变化记成 `developer/message` 事件（`role: 'developer'`，内容为 `tool-addition` / `tool-removal` 块）并给请求附带 `toolHistory`（`dsh-llm` 的 `GenerateOptions.toolHistory?`、`dsh-session` 的 `Session.toolHistory()` 都是新增可选面）。这些是宿主侧的登记动作，不是 Agent 的工作证据，因此 `session.ts` / `router.ts` / `auto.ts` 的事件与块分发（`if/else` 链，无 `assertNever`）**刻意**让它们落空——**不要**为"读全事件"补分支，也不要把它们加进 `textOf()` / `blockText()` / `isEvidenceOutput()`。`auto.ts` 的 `stale()` 只统计有结果的工具调用与 PTC 派发，所以任务中途追加的 `developer/message` 不会让已完成的验收"过期"。(b) DeepSeek provider 拆成 `@deepseek-ai/dsh-llm-deepseek-api-key` 与 `@deepseek-ai/dsh-llm-deepseek-account` 两个包，但 profile 里的**条目 id 仍是 `llm-deepseek`**，设置命名空间与其中的 `baseURL` / `apiKeyEnv` 都没变——`top-logprobs.ts` 照旧读 `llm-deepseek`，**不要**改读新包名。
- **`peerDependencies` 必须覆盖实际运行线，且 0.1.7-rc.1 起宿主会强制这道检查**：semver 的 `^0.1.7` **不匹配** `0.1.7-alpha.1` 这类预发布版，新增版本线要照现有写法显式列出预发布形态（当前各包以 `… || ^0.1.6-alpha.1 || ^0.1.6-alpha.2 || ~0.1.6 || ^0.1.7-alpha.1 || ^0.1.7-alpha.2 || ^0.1.7-rc.1 || ^0.1.7-rc.2` 收尾）。宿主侧 `packages/boot/app-boot/src/plugin-compatibility.ts` 对**每一个** `@deepseek-ai/dsh` / `@deepseek-ai/dsh-*` 条目跑 `semver.satisfies(runtimeVersion, range, { includePrerelease: true })`（运行版本取 `app-boot` 自己的 `package.json`），不满足就把该行标 `disabled`（`profile.ts` 对 bundle 层同理，直接跳过整个 bundle 并打 stderr）。`@deepseek-ai/cordis` 不在检查范围内。因此漏列一条宿主版本线的后果**不是**降级运行，而是**整个插件在 profile 里被停用**；用户只能靠 `dsh plugin allow-version` 的精确版本豁免（profile 目录下的 `compatibility.json`）临时放行。升级宿主后第一件事就是确认新版本线已在 peer 范围内。
