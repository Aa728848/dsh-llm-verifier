...
- **失败行必须保留失败前已成功与已在途的用量**：partial 载体在 `caller.ts`（`partialUsage`/`attachUsage`，`engine.partialStats()` 读取）；`engine.ts` 在 job/repeat 的 worker 里累计，`mapLimited` 首个失败后停止发起新工作但**等待在途请求落地**、对累计 `finishStats` 后再抛，`select` 在 pivot 阶段前先并入 ring 阶段；`retrying` 在 abort 时把 attempt 挂到真正抛出的 `signal.reason`、重试成功后给返回的 `usage.usageIncomplete` 置真（前面尝试的 token 未知），`unusable()` 给截断/空文本/stream 失败的响应挂上已计费 usage；裁判失败统一走 `accountFailure()`（有 partial 合并，否则 attempts + retries）。**别再回到"所有 job 完成后才汇总"或"`Promise.all` 立即 reject"**——前者把 2 成功 + 1 失败记成 1 次、0 token，后者丢掉在途请求的用量。
