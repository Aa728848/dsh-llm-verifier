...
import { StatisticsStore, emptyRunStats, errorDetails, resolveStatisticsFile, type InvocationRecord, type StatisticsOverview, type VerifierToolName } from './statistics.ts'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const record = async <T>(toolName: VerifierToolName, exec: { agent?: { id: string } }, operation: () => Promise<{ result: T; selected: { provider: string; model: string } }>): Promise<T & { provider: string; model: string }> => {
    ...
    try {
      ...
      await statistics.record({ toolName, ...(exec.agent ? { sessionId: String(exec.agent.id) } : {}), startedAt, success: true, provider: selected.provider, model: selected.model, stats: statsFrom(value) }).catch(() => {})
      return value
    } catch (error) {
      const details = errorDetails(error)
      await statistics.record({ toolName, ...(exec.agent ? { sessionId: String(exec.agent.id) } : {}), startedAt, success: false, ...details, provider: selected.provider, model: selected.model, stats: emptyRunStats() }).catch(() => {})
      throw error
    }
  }
  ctx.effect(() => ctx.connection.rpc.handle('/llm-verifier', async (endpoint, payload) => {
