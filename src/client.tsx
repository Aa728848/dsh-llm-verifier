...
export function StatisticsPage({ sessionId, rpc }:StatisticsPageProps) {
  const lang = useLanguage()
  const t = dictionaries[lang]
  ...
  useEffect(()=>{const controller=new AbortController();setLoading(true);setError(null);void rpc.call('/llm-verifier','statistics',{fromMs:startOfRange(days),toMs:endOfToday(),timezoneOffsetMinutes:new Date().getTimezoneOffset(),recentLimit:50,...(sessionOnly?{sessionId:String(sessionId)}:{})},controller.signal).then((result: { ok: boolean; value?: unknown; error?: { message: string } })=>{if(!result.ok)throw new Error(result.error?.message??t['stats.requestFailed']);setData(result.value as StatisticsOverview)},(cause: unknown)=>{if(!controller.signal.aborted)throw cause}).catch((cause: unknown)=>{if(!controller.signal.aborted)setError(message(cause))}).finally(()=>{if(!controller.signal.aborted)setLoading(false)});return()=>controller.abort()},[days,sessionOnly,sessionId,refresh,rpc,t])
  ...
  return <main style={{height:'100%',overflow:'auto',boxSizing:'border-box',padding:'22px clamp(16px, 3vw, 38px) 48px',color:'var(--dsw-text-primary)',background:'radial-gradient(circle at 10% 0%, rgba(115,77,255,.09), transparent 32%), radial-gradient(circle at 100% 8%, rgba(47,197,201,.07), transparent 28%)'}}>
    <div style={{maxWidth:1180,margin:'0 auto',display:'flex',flexDirection:'column',gap:16}}>
      <header style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:16,flexWrap:'wrap'}}><div><div style={{display:'flex',alignItems:'center',gap:10}}><span style={{display:'grid',placeItems:'center',width:34,height:34,borderRadius:10,background:'rgba(79,140,255,.14)',color:'#6da0ff'}}><IconDataOutline16 size={18}/></span><h2 style={{margin:0,fontSize:23}}>{t['stats.pageTitle']}</h2></div><p style={{margin:'7px 0 0 44px',...muted}}>{tFormat(t['stats.updatedAt'], { time: data ? dateTime(data.generatedAt, lang) : '--' })}</p></div>
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><div style={{display:'flex',padding:3,borderRadius:10,background:'var(--dsw-surface-sunken)',border:'1px solid var(--dsw-alias-border-l2, rgba(255,255,255,.12))'}}>{[7,30,90].map(value=><button key={value} onClick={()=>setDays(value)} style={{border:0,borderRadius:7,padding:'6px 10px',cursor:'pointer',color:days===value?'#fff':'var(--dsw-text-secondary)',background:days===value?'#3f68d8':'transparent'}}>{tFormat(t['stats.daysUnit'], { days: value })}</button>)}</div><button onClick={()=>setSessionOnly(value=>!value)} style={{border:'1px solid var(--dsw-alias-border-l2, rgba(255,255,255,.15))',borderRadius:9,padding:'7px 11px',cursor:'pointer',color:'var(--dsw-text-primary)',background:sessionOnly?'rgba(79,140,255,.18)':'var(--dsw-surface-sunken)'}}>{sessionOnly?t['stats.currentSession']:t['stats.allSessions']}</button><button title={t['stats.refresh']} onClick={()=>setRefresh(value=>value+1)} style={{display:'grid',placeItems:'center',width:34,height:34,borderRadius:9,border:'1px solid var(--dsw-alias-border-l2, rgba(255,255,255,.15))',color:'var(--dsw-text-primary)',background:'var(--dsw-surface-sunken)',cursor:'pointer'}}><IconRefreshOutline16 size={16}/></button></div>
      </header>
      ...
      {loading&&!data?<div style={{...dashboardCard,padding:32,textAlign:'center',...muted}}>{t['stats.loading']}</div>:<>
        <section style={{...dashboardCard,padding:'22px 24px',display:'grid',gridTemplateColumns:'minmax(220px,1.4fr) minmax(240px,1fr)',gap:24,alignItems:'center'}}><div><div style={muted}>{tFormat(t['stats.costSummary'], { days })}</div><div style={{fontSize:38,fontWeight:780,letterSpacing:'-.03em',margin:'5px 0'}}>{money(totals?.estimatedCostUsd??0)}</div><div style={muted}>{tFormat(t['stats.callsSummary'], { invocations: compact(totals?.invocations??0, lang), calls: compact(totals?.calls??0, lang) })}</div></div><div style={{display:'grid',gridTemplateColumns:'1fr auto',gap:'9px 16px',fontSize:13}}><span style={muted}>{t['stats.successCalls']}</span><strong>{compact(totals?.successes??0, lang)} <small style={{color:'#77d49b'}}>▲ {((totals?.successRate??0)*100).toFixed(1)}%</small></strong><span style={muted}>{t['stats.failedCalls']}</span><strong>{compact(totals?.failures??0, lang)}</strong><span style={muted}>{t['stats.avgDuration']}</span><strong>{duration(totals?.averageDurationMs??0)}</strong></div></section>
