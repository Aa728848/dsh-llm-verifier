...
const toolLabels: Record<string, string> = { verifier_route_classify: '路由分类', verifier_compare: '两项对比', verifier_select: '多项优选', verifier_track: '进度跟踪', verifier_current_session: '会话验收' }
...
function VerifierSettings({ remote }: VerifierSettingsProps) {
  ...
  return <div style={shell}>
    ...
    <section style={group}><GroupTitle>自动验收</GroupTitle>
      <div style={row}><Label title="调用策略" help="手动仅暴露工具；智能在有结构化/高置信证据时路由四工具；严格还会主动执行语义路由并对任何关键操作做最终验收。"/><select style={selectStyle} value={draft.autoVerifyMode} onChange={e=>patch('autoVerifyMode',e.target.value as Values['autoVerifyMode'])}><option value="manual">手动</option><option value="smart">智能（推荐）</option><option value="strict">严格</option></select></div>
      <div style={row}><Label title="混合语义路由" help="结构化候选或检查点不足时，由裁判模型保守识别真实的 compare/select/track 对象。智能模式仅在有候选线索时运行；严格模式每次结束边界都会检查。"/><button type="button" role="switch" aria-checked={draft.autoRouteSemantic} aria-label="混合语义路由" onClick={()=>patch('autoRouteSemantic',!draft.autoRouteSemantic)} style={toggleStyle(draft.autoRouteSemantic)}><span style={toggleThumbStyle(draft.autoRouteSemantic)}/></button></div>
      <div style={row}><Label title="语义路由置信度" help="语义识别达到该置信度才执行 compare/select/track；范围 0–1，建议保持 0.9。"/>{numeric('autoRouteMinConfidence',0)}</div>
      <div style={row}><Label title="最多候选数" help="一次自动 select 最多纳入的真实候选数量，至少 3。"/>{numeric('autoRouteMaxCandidates',3)}</div>
      <div style={row}><Label title="每任务最多路由" help="compare/select/track 自动路由的任务预算；指纹去重后仍受此上限约束。"/>{numeric('autoRouteMaxPerTask',1)}</div>
      <div style={row}><Label title="每会话最多路由" help="同一会话中 compare/select/track 自动路由总预算。"/>{numeric('autoRouteMaxPerSession',1)}</div>
      <div style={row}><Label title="进度完成阈值" help="track 任一检查点低于该值时 steering 要求 Agent 继续执行；范围 0–1。"/>{numeric('autoTrackCompletionThreshold',0)}</div>
      <div style={row}><Label title="单项证据字符上限" help="每个候选或检查点在脱敏后允许发送给裁判的最大字符数。"/>{numeric('autoRouteMaxItemChars',100)}</div>
      <div style={row}><Label title="路由证据总字符上限" help="一次 compare/select/track 所有候选或步骤的总字符预算。"/>{numeric('autoRouteMaxInputChars',1000)}</div>
      <div style={row}><Label title="每任务模型调用预算" help="语义分类、中间路由和最终验收共享的估算模型调用上限。"/>{numeric('autoMaxModelCallsPerTask',1)}</div>
      <div style={row}><Label title="每会话模型调用预算" help="整个会话所有自动验证阶段共享的估算模型调用上限。"/>{numeric('autoMaxModelCallsPerSession',1)}</div>
      ...
      {draft.autoVerifyMode!=='manual'&&<p style={{margin:'8px 0 0',fontSize:12,lineHeight:'18px',color:'var(--dsw-alias-state-warn-label)'}}>自动路由按 select → compare → track → current_session 的阶段顺序处理；脱敏后的任务、Assistant 轨迹与真实工具输出会发送给所选裁判模型。候选选择或进度不足会 steering 继续工作，最终验收未通过会要求修复并重新验证。</p>}
    </section>
