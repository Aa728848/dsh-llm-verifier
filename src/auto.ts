...
const USER_QUESTION_PATTERNS: readonly RegExp[] = [
  ...
  // Permission offers — "要我…吗？" / "want me to…?". These are the commonest pause in practice and
  // the list above missed every one of them: the question mark sits MID-message because the model
  // keeps justifying after asking, so the trailing text (which the whole-message match still scans)
  // decides nothing. Measured over the turn-final message of every turn in 20 recorded sessions:
  // 44 tool-less messages, of which the old list caught 8 and these three patterns bring to 13 —
  // five real pauses recovered, and not one delivery report misread as a pause.
  /(?:需要|要不要|要)我[^。！!？?\n]{0,80}[？?]/u,
  /\b(?:want|like)\s+me\s+to\b[^.!?\n]{0,80}\?/iu,
  /(?:等你|等您)(?:决定|拍板|回复|答复|确认)/u,
]
