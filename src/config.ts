...
const volatileRefSchemas = new WeakSet<object>()

/** Whether a value implements the shared cosmokit volatile reference protocol. */
export function isVolatileConfigRef(value: unknown): value is VolatileConfigRef<unknown> {
 ...
 */
function freezeVolatileSnapshot(value: unknown, ancestors = new Set<object>()): unknown {
  ...
  try {
    ...
    return Object.freeze(Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, freezeVolatileSnapshot(item, ancestors)]),
    ))
  } finally {
 ...
 */
export function createVolatileConfigRef<T>(value: T): VolatileConfigRef<T> {
  let current = freezeVolatileSnapshot(value) as T
  const ref: Record<PropertyKey, unknown> = { get: () => current }
  Object.defineProperty(ref, VOLATILE_WRITE, {
    value: (next: unknown) => { current = next as T },
    enumerable: false,
  })
  return Object.freeze(ref) as unknown as VolatileConfigRef<T>
}
 ...
 */
export function installVolatileRefProtocol<T extends z>(schema: T): T {
  if (volatileRefSchemas.has(schema as unknown as object)) return schema
  const standard = (schema as unknown as {
    '~standard'?: { version?: number; vendor?: string; validate?: (value: unknown) => unknown }
  })['~standard']
  if (!standard || typeof standard.validate !== 'function') return schema
  volatileRefSchemas.add(schema as unknown as object)
  const validate = standard.validate
  const shadow = {
    version: standard.version ?? 1,
    vendor: standard.vendor ?? 'schemastery',
    validate(value: unknown) {
      const result = validate(value) as { value?: unknown; issues?: unknown } | undefined
      // A rejection stays a rejection: the fail-closed path must never be wrapped into a value.
      if (!result || typeof result !== 'object' || 'issues' in result) return result
      if (isVolatileConfigRef(result.value)) return result
      return { ...result, value: createVolatileConfigRef(result.value) }
    },
  }
  Object.defineProperty(schema, '~standard', { value: shadow, enumerable: false, configurable: true })
  return schema
}
...
export function markVolatile<T extends z>(schema: T): T {
  ...
  if (schema.meta?.volatile) {
    // Already marked: host `.volatile()` throws on a second call, so never re-enter it.
    marked = schema
  } else if (typeof (schema as { volatile?: unknown }).volatile === 'function') {
    marked = (schema as unknown as { volatile(): T }).volatile()
  } else {
