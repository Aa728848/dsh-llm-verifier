...
## 4. 与 `README.md` / `docs/` 的分工

- **Agent Note**：单次变更的「问题 → 决定 → 备选 → 后果」与验证基线，随变更一起演进。
- **`README.md` / `docs/`**：长期有效的使用说明与专题设计文档（如 `docs/upstream-turboagent-review.md`、`docs/plan-a-best-of-n.md`）。
- 两者可以互相引用，但不要复制两份；长文留在 `docs/`，笔记只记录本次变更的判断与证据。

> 本体系于 2026-09-13 引入。首批两篇笔记（生成侧工具与其截断修复）属于引入时的一次性补录——
> 对应代码先于本目录落地；此后的变更严格执行「第 3 节 第 1 条」。