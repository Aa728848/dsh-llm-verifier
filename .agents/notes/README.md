...
目录形态与分类口径参考同级项目 `cwtools-vscode` 的 `.agents/notes/`。
...
- **`{lifecycle}`（生命周期）**：
  ...
  - `implemented/`：已实现并入库（`Status: implemented`）。
...
- **`YYYY-MM-DD-slug.md`（文件名）**：
  - 以变更日期开头（例如 `2026-09-13`）。
  - 后接简明英文短横线命名（slug），如 `best-of-n-generation-side-selection.md`。
...
4. **交付态写事实**：`implemented/` 下的笔记是交付物的一部分，必须如实反映实际落地的代码、测试与验证基线；未验证的推断要标明。
5. **必须使用中文书写**：标题与正文使用规范的简体中文（代码符号、配置项、API 名称、文件路径与 slug 除外）。必要时补充 Mermaid 流程图与输入输出图。

---

## 4. 与其他文档的分工

- **Agent Note**（本目录）：单次变更的「问题 → 决定 → 备选 → 后果」与验证基线，随变更一起演进。**这里是决策记录的唯一处所。**
- **`README.md`**：面向使用者的长期文档（能力、配置、用法）。
- **`AGENTS.md`**：面向编码代理的规矩清单（命令、硬约束、「看着像 bug 但其实是设计」）。
- `docs/` 目录目前为空：前一轮的两份专题长文（上游对照、A 计划与验收记录）已随提交 `c2b1f6d` 移除，其决策与实测数据已进入本目录（见 `implemented/process/2026-09-13-remove-docs-design-documents.md`），全文可从 `c2b1f6d^` 取回。
- 三者可以互相引用，但不要复制两份：判断与证据进笔记，长期说明进 `README.md`，规矩进 `AGENTS.md`。

> 本体系于 2026-09-13 引入。首批两篇笔记（生成侧工具与其截断修复）属于引入时的一次性补录，同一天稍后补齐了同一改进轮的其余 11 篇
> （窗口：`b763333` 至 `2cd9c39`，含 `c2b1f6d` 的文档移除登记）。这些都属于体系引入时的一次性补录——对应代码先于本目录落地，此后的变更严格执行「第 3 节 第 1 条」。