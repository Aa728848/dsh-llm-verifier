...
决策依据是判别探针实测：明显质量差的候选对中位分数差 0.544（单次胜率 63.3%），而两侧都做了真实工作的难分对只有
0.018（50.4%）。结论是「做 A（显式触发的生成侧选优），不做 B（拦截 `llm/stream` 对每次调用都做 best-of-N）」。
（原先记载这些数据的两份专题文档已随 `c2b1f6d` 移除，全文可从 `c2b1f6d^` 取回；结论与实测数据已收敛到本笔记的
`Consequences` 一节与 `AGENTS.md`。）
...
- 全部真机数据记在本笔记与 `bug-fix/2026-09-13-best-of-n-draft-truncation.md`；后续改动该工具时应同步更新这两篇笔记。

> 交付提交：`e632186`（A 优于 B 的决策）、`ea6254b`（A 计划）、`c7ff704`（工具落地）、`e71013c`（schema 一致性测试 + 可信区间文档）、
> `a5226f9`（截断修复）、`64e989e`（基线比较标签）、`fa9648b`（验收记录）。

## Errata (2026-09-13)

本笔记 Problem 与 Consequences 里引用的探针指标及推论存在概念混淆，此处追加勘误（原正文作为历史决策记录保留，后续实现与文档以此勘误为准）：

1. **把 Bradley–Terry 软偏好权重读成了胜率。** 探针数据里的 0.544 / 0.018 是候选在 [0,1] 标尺上的中位分差 Δ；
   63.3% 与 50.4% 是 `sigmoid(Δ)`（`1 / (1 + exp(-Δ))`）的输出，用于锦标赛累积偏好份额，**不是**模型单次二选一的实测选对率。
   即使 Δ 取满值 1.0，映射也只有约 73.1%；引擎最终按确定性累计分排序，不按这个概率抽签。
2. **「难分对接近抛硬币」不成立。** 两组候选在 10 次重复、A/B 槽位互换下的方向正确率都是 10/10，方向稳定一致；
   难分对出现 0.5/0.5/0.5 全平的原因是 Δ=0.018 小于 explicit-tag 通道的单档步长 1/19 ≈ 0.053，而不是选择退化成了随机。
3. **分差不是置信区间，`gap ≥ 0.15` 的放宽条件作废。** 原始样本分差描述的是两个**固定候选对**，既不是统计置信区间，
   也不能外推成总体可靠性。把「是否支持 logprobs」当作能否做过程选优的前置许可同样缺乏依据；
   后续按 S05-B / P05 的真实任务对照（任务结果、重复稳定性、成本）判断，见本目录同日的 `process/2026-09-13-soft-preference-metric-errata.md`。

> 复核口径：README 的 best-of-N 使用章节与 `AGENTS.md` 同条目已同步改正；`src/` 中不存在把分差当置信区间或断言固定 gap 的逻辑。
