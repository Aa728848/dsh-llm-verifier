...
function analyzeAutoTask(events, policy) {
	...
	const codeDispatches = relevant.filter((event) => event.type === "tool/code-dispatch").map((event) => event.data);
...
function buildEvidenceIndex(events) {
	...
	for (const [callId, call] of calls) {
		...
		if (result) paired.set(callId, {
			...
			text: blockText(result.data.message.content)
		});
	}
	return {
...
function detectPlanExit(events, fromSeq = 0) {
	for (let i = events.length - 1; i >= 0; i--) {
		...
		if (event.type === "tool/call" && event.data.name === "exit_plan_mode") {
			...
			try {
				...
				if (typeof parsed.plan === "string" && parsed.plan.trim()) planText = parsed.plan.trim();
			} catch {}
			if (!planText) for (let j = i - 1; j >= 0; j--) {
				const prev = events[j];
...
function buildPlanPreReviewPrompt(problem, planText, maxChars = 2e4) {
	return [
		...
		"",
		"Task requirement:",
		sanitizeVerifierText(problem, 4e3),
		"",
		"Proposed Plan:",
...
function inspectTeamTasks(events, fromSeq = 0) {
	...
	return {
		...
		hasRecentCompletedTask: latestCompletedTask !== void 0
	};
}
...
function apply(ctx, config = {}) {
	...
	const engine = async (agent) => {
		...
		return {
			verifier: new VerifierEngine({
				...
				limiter
			}, selected.maxConcurrency, topicEntry.cache, {
				...
				handler: (req, res) => {
					...
					req.on("end", async () => {
						try {
							...
							if (isRpcEnvelope) res.end(JSON.stringify({
								type: "server-response",
								rpcId,
								result: outcome
							}));
							else res.end(JSON.stringify(outcome));
						} catch (err) {
	...
	if (typeof ctx.inject === "function") try {
		ctx.inject(["webServer"], (webCtx) => {
			if (webCtx?.webServer) mountWebServerRoute(webCtx.webServer);
		});
	} catch {}
	try {
		services.connection?.rpc?.handle("/llm-verifier", async (endpoint, payload) => {
	...
	ctx.on("agent/turn-stopping", async ({ agent, signal }) => {
		...
		if (selected.autoVerifyPlanMode) {
			...
			if (planDetection.hasExitPlanMode && planDetection.planText) {
				const planFingerprint = stableHash({
					phase: "plan_review",
					callSeq: planDetection.callSeq,
					plan: planDetection.planText
				});
				const planReservation = autoRouter.reserve(agent, "plan_review", planFingerprint, 1, policy);
				if (planReservation) try {
					const prompt = buildPlanPreReviewPrompt((await extractTask(agent, evidence.taskStartSeq, admittedLastSeq, selected.autoVerifyMaxChars, signal)).problem, planDetection.planText, selected.autoRouteMaxInputChars);
					const classified = await classifyRoute(agent, prompt, signal);
					if (!stillCurrent()) {
						autoRouter.fail(agent, planReservation, false);
						return;
					}
					const verdict = parsePlanReviewVerdict(classified.text);
					if (verdict.score >= selected.autoVerifyThreshold) autoRouter.commit(agent, planReservation, admittedLastSeq);
					else {
						...
						agent.steer(createUserMessage({
							...
							source: {
								...
								plugin: "dsh-llm-verifier",
								form: "notice",
								summary: "Plan Pre-review Feedback"
							}
						}));
						return;
					}
					...
					if (selected.autoVerifyMode === "strict" && !signal.aborted) {
						agent.steer(createUserMessage({
							content: [{
								type: "text",
								text: "[Automatic Verifier Plan Pre-review]\nPlan pre-review failed: " + (error instanceof Error ? error.message : String(error))
							}],
							source: {
								kind: "plugin",
								plugin: "dsh-llm-verifier"
		...
		if (selected.autoVerifyTeamTasks) {
			...
			if (teamInspection.hasRecentCompletedTask && teamInspection.latestCompletedTask) {
				...
				const taskFingerprint = stableHash({
					phase: "team_task",
					taskId: task.id,
					status: task.status,
					seq: teamInspection.completedSeq
				});
				const taskReservation = autoRouter.reserve(agent, "team_task", taskFingerprint, 1, policy);
				if (taskReservation) try {
					...
					const verdict = parsePlanReviewVerdict(classified.text);
					if (verdict.score >= selected.autoVerifyThreshold) autoRouter.commit(agent, taskReservation, admittedLastSeq);
					else {
...
//#endregion
export { AutoVerificationBudget, AutoVerifierRouter, Config, DEFAULT_CRITERIA, DEFAULT_GROUND_TRUTH_NOTE, GRANULARITY, LETTERS, RequestLimiter, SCALE_DESCRIPTION, ScoreCache, SingleFlight, StatisticsStore, VERIFIER_TOOL_NAMES, VerifierEngine, accumulatePairs, analyzeAutoTask, analyzeStructuredRoute, apply, automaticFeedback, boundDecision, bradleyTerry, buildEvidenceIndex, buildPairwisePrompt, buildPlanPreReviewPrompt, buildProgressPrompt, buildSemanticRoutePrompt, buildTeamTaskVerificationPrompt, callVerifier, detectPlanExit, emptyRunStats, errorDetails, extractProgressScore, extractScore, inject, inspectTeamTasks, latestDirectUserSeq, mergeStatisticsOverviews, name, normalizeCriteria, normalizeScoreLetter, parsePlanReviewVerdict, parseSemanticRoute, pivotRoundPairs, rankScores, resolveCacheFile, resolveStatisticsFile, resolveTopicDataDir, ringCycle, seededRandom, semanticDecision, semanticRouteHint, stableHash, topPivots };
