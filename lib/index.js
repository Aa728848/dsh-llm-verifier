...
import { a as callVerifier, c as generateCandidate, d as TopLogprobCapabilityCache, f as resolveCapabilityFile, i as addUsage, o as callVerifierText, r as RequestLimiter, s as emptyUsage, u as predictScoringChannel } from "./caller-DR_C7ueP.js";
import { C as ScoreCache, E as stableHash, S as sessionEvents, T as resolveCacheFile, _ as semanticDecision, a as sessionAccepted, b as extractSession, c as analyzeStructuredRoute, d as buildSemanticRoutePrompt, f as buildSemanticRouteView, g as routedRepeats, h as parseSemanticRoute, i as isSubagentSession, l as boundDecision, m as latestDirectUserSeq, n as automaticFeedback, o as AutoVerifierRouter, p as estimateRoutedCalls, r as failedAcceptanceCriteria, s as MAX_ROUTED_CHECKPOINTS, t as analyzeAutoTask, u as buildEvidenceIndex, v as semanticReferencesVisible, w as SingleFlight, x as sanitizeVerifierText, y as semanticRouteHint } from "./auto-fG6M4Bya.js";
...
function apply(ctx, config = {}) {
	...
	*/
	const bestOfN = async (agent, task, count, repeats, signal, criteriaInput) => record("verifier_best_of_n", agent, async (trace) => {
		...
		if (survivors.length < MIN_BEST_OF_N) throw new Error("llm-verifier: best-of-n produced " + survivors.length + " usable draft(s) out of " + count + "; at least 2 are required to choose between them" + (failures.length === 0 ? "" : " — " + failures.join("; ")));
		const ranked = await verifier.select({
			problem,
		...
		stats.estimatedCostUsd = ((stats.inputTokens + stats.cachedInputTokens) * selected.estimatedInputUsdPerMillion + stats.outputTokens * selected.estimatedOutputUsdPerMillion) / 1e6;
		return {
			result: {
				best: ranked.best,
				index: ranked.index,
	...
	*/
	...
	ctx.on("agent/turn-stopping", async ({ agent, signal }) => {
		...
		if (!finalPreferred && decision === void 0 && selected.autoRouteSemantic && (selected.autoVerifyMode === "strict" || semanticRouteHint(snapshot))) {
			...
			if (reservation) try {
				const view = buildSemanticRouteView((await extractTask(agent, evidence.taskStartSeq, admittedLastSeq, selected.autoVerifyMaxChars, signal)).problem, snapshot, selected.autoRouteMaxCandidates, selected.autoRouteMaxItemChars, selected.autoRouteMaxInputChars);
				...
				if (parsed.confidence < selected.autoRouteMinConfidence) await recordSkippedRoute(agent, "none", "semantic", parsed.kind === "none" ? "none" : "low-confidence");
				if (parsed.confidence >= selected.autoRouteMinConfidence) if (parsed.kind !== "none" && !semanticReferencesVisible(parsed, view)) {
