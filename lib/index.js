import { a as emptyUsage, c as resolveCapabilityFile, i as callVerifierText, n as addUsage, o as predictScoringChannel, r as callVerifier, s as TopLogprobCapabilityCache, t as RequestLimiter } from "./caller-DJd_FlrQ.js";
...
function buildEvidenceIndex(events) {
	...
	let narration;
	const dispatchedTools = /* @__PURE__ */ new Map();
	for (const rawEvent of relevant) {
		...
		if (event.type === "tool/call") calls.set(String(event.data.callId), rawEvent);
			...
			if (rootCallId) {
				const names = dispatchedTools.get(rootCallId) ?? /* @__PURE__ */ new Set();
				names.add(data.name);
...
function onlyBookkeepingDispatches(names) {
	if (names === void 0 || names.size === 0) return false;
...
function semanticDecision(output, events, maxItemChars = 2e4, maxInputChars = 6e4) {
	...
	const candidates = output.candidateCallIds.map((callId, i) => {
		...
		if (!pair || BOOKKEEPING_TOOLS.has(pair.name)) return void 0;
		return {
			id: callId,
			groupId: "semantic",
			label: pair.name + " " + (i + 1),
...
*/
function routedRepeats(decision, configured) {
...
function analyzeAutoTask(events, policy) {
	...
	const manualVerificationAccepted = manualVerdicts.some((verdict) => verdict.winner === "A" && typeof verdict.score === "number" && sessionAccepted({
		score: verdict.score,
		winner: "A",
		criteria: verdict.criteria
	}, policy.threshold) && !stale(verdict.seq));
...
function summarizeVerdict(toolName, value, phase, thresholds) {
	...
	const criteria = Array.isArray(row.criteria) ? row.criteria.map((entry) => typeof entry === "object" && entry !== null ? entry : {}).filter((entry) => typeof entry.id === "string" && typeof entry.scoreA === "number" && Number.isFinite(entry.scoreA)).slice(0, MAX_VERDICT_CRITERIA).map((entry) => ({
		id: entry.id,
		score: entry.scoreA
	})) : [];
...
function apply(ctx, config = {}) {
	...
	*/
	...
	const verifySession = async (agent, options, signal, phase = "explicit") => record("verifier_current_session", agent, async () => {
	...
	const compareCandidates = async (agent, problem, candidateA, candidateB, repeats, signal, routedImages = [], phase = "explicit") => record("verifier_compare", agent, async () => {
		const { verifier, selected } = await engine(agent);
		return {
	...
	const selectCandidates = async (agent, problem, candidates, repeats, signal, routedImages = [], phase = "explicit") => record("verifier_select", agent, async () => {
		const { verifier, selected } = await engine(agent);
		return {
	...
	const trackProgress = async (agent, problem, steps, checkpoints, repeats, signal, routedImages = [], phase = "explicit") => record("verifier_track", agent, async () => {
		const { verifier, selected } = await engine(agent);
		return {
	...
	const classifyRoute = async (agent, prompt, signal, phase) => record("verifier_route_classify", agent, async () => {
		const { verifier, selected } = await engine(agent);
