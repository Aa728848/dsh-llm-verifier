import { a as emptyUsage, c as resolveCapabilityFile, i as callVerifierText, n as addUsage, o as predictScoringChannel, r as callVerifier, s as TopLogprobCapabilityCache, t as RequestLimiter } from "./caller-D1ZQNsVU.js";
import { DEFAULT_CRITERIA, DEFAULT_GROUND_TRUTH_NOTE, GRANULARITY, LETTERS, SCALE_DESCRIPTION, UNTRUSTED_EVIDENCE_NOTE, accumulatePairs, bradleyTerry, buildPairwisePrompt, buildProgressPrompt, extractProgressScore, extractScore, normalizeScoreLetter, pivotRoundPairs, rankScores, ringCycle, seededRandom, topPivots } from "./core.js";
...
function isBlockedImageHost(hostname) {
	...
	if (host.includes(":")) return host === "::1" || /^fe80:/i.test(host) || /^f[cd]/i.test(host);
...
function analyzeAutoTask(events, policy) {
	...
	const codeDispatches = relevant.filter((event) => CODE_DISPATCH_TYPES.has(event.type)).map((event) => event.data);
