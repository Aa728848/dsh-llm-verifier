...
function apply(ctx, config = {}) {
	...
	*/
	...
	ctx.on("agent/pre-step", async (payload, next) => {
		...
		if (inspectUserInteractionPause(sessionEvents(payload.agent.session), latestDirectUserSeq(sessionEvents(payload.agent.session)) ?? 0, payload.turn) !== void 0) return decision;
