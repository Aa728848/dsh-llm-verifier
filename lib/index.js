...
function boundDecisionCalls(calls) {
	...
	for (const call of calls.slice(0, MAX_CALLS)) {
		const value = {
			...
			prompt: sanitizeVerifierText(call.prompt, MAX_PROMPT_CHARS),
			output: sanitizeVerifierText(call.output, MAX_OUTPUT_CHARS),
...
function apply(ctx, config = {}) {
	...
	const record = async (toolName, agent, operation, phase = "explicit") => {
		...
		try {
			...
			if (calls.length > 0) await topicEntry.decisions.record({
				...
				provider: selected.provider,
				model: selected.model,
				calls: boundDecisionCalls(calls)
			}).catch(() => {});
			return value;
		} catch (error) {
	...
	*/
	...
	const verifySession = async (agent, options, signal, phase = "explicit") => record("verifier_current_session", agent, async (trace) => {
	...
	const compareCandidates = async (agent, problem, candidateA, candidateB, repeats, signal, routedImages = [], phase = "explicit") => record("verifier_compare", agent, async (trace) => {
		const { verifier, selected } = await engine(agent);
		return {
	...
	const selectCandidates = async (agent, problem, candidates, repeats, signal, routedImages = [], phase = "explicit") => record("verifier_select", agent, async (trace) => {
		const { verifier, selected } = await engine(agent);
		return {
	...
	if (anyConn && anyConn.fetch && typeof anyConn.fetch.register === "function") try {
		anyConn.fetch.register({
			...
			fetch: async (request) => {
				...
				const outcome = isDecisionQuery(payload) ? await handleDecisionQuery(payload.id) : await handleStatisticsQuery(payload);
