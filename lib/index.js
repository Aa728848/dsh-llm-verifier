...
import { a as attachUsage, c as emptyUsage, f as partialUsage, g as resolveCapabilityFile, h as TopLogprobCapabilityCache, i as addUsage, m as requestAttempts, o as callVerifier, p as predictScoringChannel, r as RequestLimiter, s as callVerifierText, u as generateCandidate } from "./caller-BUK7OgUs.js";
...
var ProcessActivities = class {
	...
	*/
	finish(sessionId, cycleId, outcome, replayed, now) {
		const current = this.active.get(sessionId);
		const known = current?.cycleId === cycleId ? current : this.settled.get(sessionId)?.cycleId === cycleId ? this.settled.get(sessionId) : void 0;
		if (known === void 0) return false;
		if (current?.cycleId === cycleId) this.active.delete(sessionId);
	...
	*/
	read(sessionId, now) {
		...
		return {
			...
			...current === void 0 ? {} : { settled: current }
		};
	}
...
var ProcessSelector = class {
	...
	*/
	activities = new ProcessActivities();
	constructor(deps) {
		this.deps = deps;
	}
	/**
...
function apply(ctx, config = {}) {
	...
	*/
	const handleProcessQuery = async (payload) => {
		...
		return rpcSuccess({
			sessionId,
			...processSelector.activity(sessionId)
		});
	};
	/**
