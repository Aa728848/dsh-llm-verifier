...
/** Judge calls one explicit selection may plan; priced by estimateRoutedCalls(). */
	...
	*/
	const verdictFrom = (toolName, value, phase) => {
		...
		if (toolName === "verifier_track") {
			const worst = scores.length > 0 ? Math.min(...scores) : void 0;
			const threshold = selected.autoTrackCompletionThreshold;
			return {
				phase,
				outcome: worst !== void 0 && worst >= threshold ? "passed" : "below-threshold",
				...worst !== void 0 ? { score: worst } : {},
				threshold
			};
		}
		...
		return {
			phase,
			outcome: winner === "tie" ? "tie" : winner === "A" && score !== void 0 && score >= threshold ? "passed" : "below-threshold",
			...score !== void 0 ? { score } : {},
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			return record("verifier_select", agent, async () => {
				...
				const planned = estimateRoutedCalls({
					kind: "select",
					source: "semantic",
					confidence: 1,
					reason: "explicit tool budget",
					fingerprint: "",
