...
import { A as sessionEvents, C as parseSemanticRoute, D as semanticRouteHint, E as semanticReferencesVisible, M as SingleFlight, N as resolveCacheFile, O as extractSession, P as stableHash, S as nextDiagnosticCycleId, T as semanticDecision, _ as buildSemanticRouteView, a as compareRouteFeedbackDetail, b as inspectRecoverySignal, c as selectRouteFeedbackDetail, d as AutoVerifierRouter, f as MAX_ROUTED_CHECKPOINTS, g as buildSemanticRoutePrompt, h as buildEvidenceIndex, i as automaticFeedback, j as ScoreCache, k as sanitizeVerifierText, l as sessionAccepted, m as boundDecision, n as PROPOSAL_FEEDBACK_NOTE, o as failedAcceptanceCriteria, p as analyzeStructuredRoute, r as analyzeAutoTask, s as isSubagentSession, t as MAX_ROUTE_FEEDBACK_CHARS, u as topScoreIndices, v as estimateRoutedCalls, w as routedRepeats, x as latestDirectUserSeq, y as inspectDeliveryPhase } from "./auto-C10Y8714.js";
...
function renderCandidateView(candidate, budget) {
	...
	return ("Tool calls:\n" + (actions.length > actionBudget ? actions.slice(0, Math.max(0, actionBudget - 1)) + "…" : actions) + "\n\nReply text:\n" + (prose || "(empty)")).slice(0, Math.max(1, budget));
}
	...
	*/
	async *handle(options, next, intent) {
		...
		try {
			...
			problem.length > settings.maxItemChars && problem.slice(0, settings.maxItemChars);
			if (renderCandidateView(rendered, perCandidate) === renderCandidateView(alternative, perCandidate)) {
				router.commit(intent.agent, reservation, intent.lastSeq);
				observation.sameCandidate = true;
				for (const chunk of original) yield chunk;
				await this.report({
			...
			if (phase.signal.aborted || !this.deps.current(intent)) {
				...
				await this.report({
					...
					usage,
					compare: compared,
					...phase.signal.aborted ? { error: "the process-selection phase was cancelled" } : {}
				});
				return;
			}
			router.commit(intent.agent, reservation, intent.lastSeq);
...
function apply(ctx, config = {}) {
	...
	*/
	...
	const routePolicy = (selected, minFinalModelCalls) => ({
		...
		maxProcessPerSession: MAX_PROCESS_PER_SESSION
	});
	/**
