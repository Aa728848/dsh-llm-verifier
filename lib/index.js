import { CRITERIA_PRESETS, CRITERIA_PRESET_IDS, DEFAULT_CRITERIA, DEFAULT_GROUND_TRUTH_NOTE, EMPTY_WORK_BASELINE, GRANULARITY, LETTERS, MAX_DIAGNOSTICS, MAX_DIAGNOSTIC_ACTION_CHARS, MAX_DIAGNOSTIC_CHARS, PROPOSAL_CRITERIA, SCALE_DESCRIPTION, UNTRUSTED_EVIDENCE_NOTE, accumulatePairs, bradleyTerry, buildFindingContract, buildGenerationPrompt, buildPairwisePrompt, buildProgressPrompt, dedupeCriterionId, diagnosticKey, evidenceNonce, extractProgressScore, extractScore, normalizeScoreLetter, parseCriteriaMarkdown, parseDiagnostics, pivotRoundPairs, rankScores, renderDelimitedBlock, renderDiagnostics, renderReferenceContext, ringCycle, seededRandom, slugCriterionId, swapDiagnosticEvidence, topPivots } from "./core.js";
import { a as attachUsage, c as emptyUsage, d as partialUsage, f as predictScoringChannel, h as resolveCapabilityFile, i as addUsage, l as generateCandidate, m as TopLogprobCapabilityCache, o as callVerifier, p as requestAttempts, r as RequestLimiter, s as callVerifierText } from "./caller-Ci1zr9q7.js";
import { A as sanitizeVerifierText, C as nextDiagnosticCycleId, D as semanticReferencesVisible, E as semanticDecision, F as stableHash, M as ScoreCache, N as SingleFlight, O as semanticRouteHint, P as resolveCacheFile, S as latestDirectUserSeq, T as routedRepeats, _ as buildSemanticRouteView, a as compareRouteFeedbackDetail, b as inspectRecoverySignal, c as selectRouteFeedbackDetail, d as AutoVerifierRouter, f as MAX_ROUTED_CHECKPOINTS, g as buildSemanticRoutePrompt, h as buildEvidenceIndex, i as automaticFeedback, j as sessionEvents, k as extractSession, l as sessionAccepted, m as boundDecision, n as PROPOSAL_FEEDBACK_NOTE, o as failedAcceptanceCriteria, p as analyzeStructuredRoute, r as analyzeAutoTask, s as isSubagentSession, t as MAX_ROUTE_FEEDBACK_CHARS, u as topScoreIndices, v as estimateRoutedCalls, w as parseSemanticRoute, x as itemBudget, y as inspectDeliveryPhase } from "./auto-DHwTtyKc.js";
...
*/
function buildAlternativeRequest(options, signal) {
	return {
...
function buildProcessView(input) {
	...
	const sections = [];
	if (input.evidence !== void 0 && input.evidence.trim() !== "") sections.push("RECENT EXECUTION EVIDENCE:\n" + input.evidence.trim());
	if (input.constraints !== void 0 && input.constraints.trim() !== "") sections.push("REQUEST CONSTRAINTS:\n" + input.constraints.trim());
	if (input.tools !== void 0 && input.tools.trim() !== "") sections.push("AVAILABLE TOOLS:\n" + input.tools.trim());
	const context = sections.length === 0 ? void 0 : input.sanitize(sections.join("\n\n"), input.maxItemChars).trim();
	const fixed = task.length + (context === void 0 ? 0 : context.length);
	if (fixed > input.maxInputChars) return {
		ok: false,
		reason: "the task and its constraints need " + fixed + " characters but the process comparison budget is " + input.maxInputChars + "; replaying the original reply"
	};
	...
	const render = (candidate) => renderCandidateView({
	...
	if (candidateA === void 0) return {
		ok: false,
		reason: "candidate A has more tool-call text than the " + perCandidate + "-character candidate budget; replaying the original reply"
	};
	const candidateB = render(input.alternative);
	if (candidateB === void 0) return {
		ok: false,
		reason: "candidate B has more tool-call text than the " + perCandidate + "-character candidate budget; replaying the original reply"
	};
	...
	*/
	async lookup(sessionId, taskStartSeq) {
		try {
			...
			return {
				ok: true,
				purchased: this.records.some((record) => record.sessionId === sessionId && record.taskStartSeq === taskStartSeq)
			};
		} catch (error) {
...
var ProcessSelector = class {
	...
	*/
	async *handle(options, next, intent) {
		...
		const expected = 1 + criteria.length * 2 * Math.max(1, this.deps.judges());
		...
		try {
			...
			if (!alternative.complete || !alternative.text && alternative.actions.length === 0) {
				...
				await this.report({
					...
					observation,
					startedAt,
					outcome: alternative.complete ? "alternative-empty" : "alternative-incomplete",
					replayed: "original",
					generatedCalls: 1,
					judgeCalls: 0,
					sameCandidate: false,
					usage: statsWith(generatedUsage)
				});
				return;
			}
			...
			await this.report({
				...
				observation,
				startedAt,
				outcome: replaced ? "candidate-selected" : compared.winner === "tie" ? "tie" : "original-selected",
...
function apply(ctx, config = {}) {
	...
	*/
	...
	const routePolicy = (selected, minFinalModelCalls) => ({
		...
		minFinalModelCalls,
		maxProcessPerTask: MAX_PROCESS_PER_TASK
	});
	/**
	...
	*/
	const processSelector = new ProcessSelector({
		...
		compare: async (request) => {
			...
			if (calls.length > 0) await topic(agent.session.header).decisions.record({
		...
		record: async (report) => {
			...
			const stats = {
				...report.usage,
				estimatedCostUsd: ((report.usage.inputTokens + report.usage.cachedInputTokens) * selected.estimatedInputUsdPerMillion + report.usage.outputTokens * selected.estimatedOutputUsdPerMillion) / 1e6
			};
			const verdict = report.compare === void 0 ? {
				phase: "process",
				outcome: report.outcome
			} : summarizeVerdict("verifier_compare", {
				...report.compare,
				reviewStage: "proposal",
				criteriaSource: "proposal"
			}, "process", {
				autoVerifyThreshold: selected.autoVerifyThreshold,
				autoTrackCompletionThreshold: selected.autoTrackCompletionThreshold
			});
	...
	ctx.on("agent/turn-stopping", async ({ agent, signal }) => {
		...
		if (decision) {
			...
			if (reservation) try {
				...
				if (forcedFromSeq === void 0 && !evidence.eligible && !gateArmed) {
...
//#endregion
export { AutoVerifierRouter, CRITERIA_PRESETS, CRITERIA_PRESET_IDS, Config, DEFAULT_CRITERIA, DEFAULT_GROUND_TRUTH_NOTE, DecisionStore, EMPTY_WORK_BASELINE, GRANULARITY, LETTERS, MAX_DIAGNOSTICS, MAX_DIAGNOSTIC_ACTION_CHARS, MAX_DIAGNOSTIC_CHARS, MAX_ROUTED_CHECKPOINTS, MAX_ROUTE_FEEDBACK_CHARS, MAX_TEAM_TASK_METADATA_CHARS, PROPOSAL_CRITERIA, PROPOSAL_FEEDBACK_NOTE, RequestLimiter, SCALE_DESCRIPTION, ScoreCache, SingleFlight, StatisticsStore, UNTRUSTED_EVIDENCE_NOTE, VERIFIER_TOOL_NAMES, VerifierEngine, accumulatePairs, analyzeAutoTask, analyzeStructuredRoute, apply, automaticFeedback, boundCaptureText, boundDecision, boundDecisionCalls, bradleyTerry, buildEvidenceIndex, buildFindingContract, buildGenerationPrompt, buildPairwisePrompt, buildPlanPreReviewPrompt, buildProgressPrompt, buildSemanticRoutePrompt, buildSemanticRouteView, buildTeamTaskVerificationPrompt, callVerifier, compareRouteFeedbackDetail, dedupeCriterionId, diagnosticKey, emptyRunStats, errorDetails, estimateRoutedCalls, evidenceNonce, extractProgressScore, extractScore, failedAcceptanceCriteria, inject, inspectDeliveryPhase, inspectRecoverySignal, inspectTeamTasks, isSubagentSession, itemBudget, latestDirectUserSeq, mergeRunStats, mergeStatisticsOverviews, name, nextDiagnosticCycleId, normalizeCriteria, normalizeScoreLetter, orientRoundPairs, parseCriteriaMarkdown, parseDiagnostics, parseSemanticRoute, parseStatisticsQuery, parseVerdictLetter, partialStats, pivotRoundPairs, planFromArguments, rankScores, renderDelimitedBlock, renderDiagnostics, renderReferenceContext, resolveCacheFile, resolveDecisionsFile, resolveStatisticsFile, resolveTopicDataDir, ringCycle, routedRepeats, seededRandom, selectRouteFeedbackDetail, semanticDecision, semanticReferencesVisible, semanticRouteHint, sessionAccepted, slugCriterionId, stableHash, summarizeVerdict, swapDiagnosticEvidence, topPivots, topScoreIndices };
