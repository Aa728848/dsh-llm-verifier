...
*/
function settingsNamespace(ctx, ns) {
	const settings = ctx.get("settings");
	if (!settings) return void 0;
	try {
		if (typeof settings.get === "function") return object(settings.get(ns));
		if (typeof settings.describe !== "function") return void 0;
		const rows = settings.describe();
		if (!Array.isArray(rows)) return void 0;
		return object(rows.map(object).find((candidate) => candidate?.ns === ns)?.value);
	} catch {
		return;
	}
}
async function resolveTopLogprobRoute(ctx, provider) {
	...
	const profile = object(object(root?.providers)?.[provider]);
...
var TopLogprobCapabilityCache = class {
	...
	*/
	forget(provider, model) {
		...
		const next = this.writing.catch(() => {}).then(() => this.ensureLoaded()).then(() => {
			this.unsupported.delete(key);
	...
	persist() {
		if (this.file === void 0) return;
		const next = this.writing.catch(() => {}).then(() => this.ensureLoaded()).then(() => this.writeDocument());
		this.writing = next;
		next.catch(() => {});
	}
	async writeDocument() {
...
*/
function fallbackDeepFreeze(value) {
	if (value instanceof AbortSignal) return value;
	if (value && typeof value === "object" && !Object.isFrozen(value)) {
		Object.freeze(value);
		for (const key of Object.keys(value)) fallbackDeepFreeze(value[key]);
	}
	return value;
}
/**
...
function failureMessage(finish) {
	if (finish.kind === "error" || finish.kind === "aborted") {
		...
		const facts = [
			failure.code,
			failure.status === void 0 ? void 0 : "HTTP " + failure.status,
			failure.requestId === void 0 ? void 0 : "request " + failure.requestId
...
async function retrying(config, signal, run) {
	...
	const spent = (billed, unknown) => ({
		calls: billed === void 0 ? 0 : billed.calls,
		attempts: attempt,
	...
	const cancel = (reason, billed, unknown) => {
		const error = abortFailure(reason, attempt);
		attachUsage(error, spent(billed, unknown));
		throw error;
	};
	while (true) {
		if (signal?.aborted) cancel(signal.reason, void 0, false);
		attempt += 1;
		...
		try {
			...
			if (signal?.aborted) cancel(signal.reason ?? error, billed, unknownFailure || billed === void 0);
			...
			if (attempt > config.maxRetries || !retryable) {
				...
				attachUsage(failure, spent(billed, unknownFailure || billed === void 0));
				throw failure;
			}
			if (billed === void 0) unknownFailure = true;
			...
			try {
				...
				cancel(signal?.reason ?? waitError, void 0, unknownFailure);
			}
		} finally {
...
//#endregion
export { attachUsage as a, emptyUsage as c, generationClient as d, partialUsage as f, resolveCapabilityFile as g, TopLogprobCapabilityCache as h, addUsage as i, fallbackDeepFreeze as l, requestAttempts as m, GENERATION_TEMPERATURE as n, callVerifier as o, predictScoringChannel as p, RequestLimiter as r, callVerifierText as s, GENERATION_MAX_TOKENS as t, generateCandidate as u };

//# sourceMappingURL=caller-Bfp4HPK_.js.map