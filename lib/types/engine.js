import { addUsage, callVerifier, emptyUsage, predictScoringChannel, requestAttempts } from "./caller.js";
import { ScoreCache, SingleFlight, stableHash } from "./cache.js";
import { DEFAULT_CRITERIA, DEFAULT_GROUND_TRUTH_NOTE, accumulatePairs, buildPairwisePrompt, buildProgressPrompt, dedupeCriterionId, extractProgressScore, extractScore, pivotRoundPairs, rankScores, ringCycle, slugCriterionId, topPivots, } from "./core.js";
...
export class VerifierEngine {
    ...
    async scoreOne(client, options, candidateA, candidateB, criterion, repeat, signal) {
        ...
        if (cache === undefined) {
            ...
            return { scores: [value.scoreA, value.scoreB], usage: value.usage, scoringMode: value.scoringMode, hit: false, channelFallback: fellBack };
        }
    ...
    async scorePairs(options, pairs, signal) {
        ...
        for (const value of values) {
            ...
            if (value.result.stats.usageIncomplete)
                stats.usageIncomplete = true;
            if (value.result.stats.channelFallbacks)
                stats.channelFallbacks = (stats.channelFallbacks ?? 0) + value.result.stats.channelFallbacks;
    ...
    async track(problem, steps, checkpoints, repeats = 2, signal, images, trace) {
        ...
        if (repeatIndices.length > 0)
            runs.push(...await this.mapLimited(repeatIndices.slice(0, 1), runRepeat));
    ...
    async select(options, signal) {
        ...
        for (const source of [ringScores.stats, roundScores.stats]) {
            ...
            if (source.channelFallbacks)
                stats.channelFallbacks = (stats.channelFallbacks ?? 0) + source.channelFallbacks;
        }
        const judges = this.clients.map((client, k) => {
