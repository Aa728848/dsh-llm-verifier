...
function parseTrustedWorkflow(value, callId, callSeq, resultSeq, maxCandidates, maxItemChars, maxInputChars, reviewStage = 'artifact') {
    ...
    for (const candidates of groups) {
        ...
        if (candidates.length >= 3) {
            if (reviewed.select.has(key))
                continue;
            const decision = { kind: 'select', source: 'structured', confidence: 1, reason: 'trusted workflow candidate envelope', fingerprint: stableHash({ kind: 'select', candidates }), candidates };
        ...
        if (reviewed.compare.has(key))
            continue;
        const decision = { kind: 'compare', source: 'structured', confidence: 1, reason: 'trusted workflow candidate envelope', fingerprint: stableHash({ kind: 'compare', candidates }), candidates: [candidates[0], candidates[1]] };
        if (options.processed?.(decision.fingerprint))
            continue;
        return decision;
    }
