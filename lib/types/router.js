...
import { sanitizeVerifierText, sessionEvents, toolResultBlocks, toolResultFailed } from "./session.js";
/**
 ...
 */
export function nextDiagnosticCycleId() {
...
function parseWorkflowResult(text) {
    ...
    if (/\[truncated: \d+ more characters\]\s*$/u.test(body))
        return undefined;
...
export function buildEvidenceIndex(events) {
    ...
    for (const rawEvent of relevant) {
        ...
        if (event.type === 'tool/call')
            ...
            if (rootCallId) {
                ...
                entry.count += 1;
                if (isEvidenceOutput(data.name, Array.isArray(data.content) ? blockText(data.content) : ''))
                    entry.evidence = true;
    ...
    for (const [callId, call] of calls) {
        ...
        if (dispatched !== undefined && dispatched.count > 0 && !dispatched.evidence)
            continue;
...
function parseTrustedWorkflow(value, callId, callSeq, resultSeq, maxCandidates, maxItemChars, maxInputChars) {
    ...
    if (envelope.protocol !== 'dsh-verifier-candidates' || typeof version !== 'number' || !TRUSTED_WORKFLOW_VERSIONS.has(version) || typeof envelope.groupId !== 'string' || !envelope.groupId.trim() || !Array.isArray(envelope.candidates))
        return undefined;
    ...
    if (version >= 2) {
        if (envelope.reviewStage !== 'proposal' && envelope.reviewStage !== 'artifact')
            return undefined;
...
function explicitReviewKeys(events) {
    const reviews = { compare: new Set(), select: new Set() };
    const index = buildEvidenceIndex(events);
    if (!index)
        return reviews;
    for (const pair of index.calls.values()) {
        if (!pair.ok || !ROUTED_TOOLS.has(pair.name) || typeof pair.args !== 'string')
            continue;
        let parsed;
        try {
        ...
        if (pair.name === 'verifier_select' && Array.isArray(row.candidates) && row.candidates.every(value => typeof value === 'string')) {
            ...
            if (contents.length >= 3)
                reviews.select.add(reviewKey(stage, contents));
            else if (contents.length === 2)
                reviews.compare.add(reviewKey(stage, contents));
        }
...
function isEvidenceOutput(name, text) {
    if (BOOKKEEPING_TOOLS.has(name) || VERIFIER_EVIDENCE_TOOLS.has(name))
        return false;
 ...
 */
export function verificationVerdict(text) {
    if (VERIFICATION_FAILURE_SIGNATURES.some(pattern => pattern.test(text)))
        return 'failed';
    if (VERIFICATION_PASS_SIGNATURES.some(pattern => pattern.test(text)))
        return 'passed';
    return undefined;
}
/**
 ...
 */
export function verificationFailed(call) {
 ...
 */
export function inspectDeliveryPhase(events) {
    const index = buildEvidenceIndex(events);
    if (!index)
        return undefined;
    ...
    let verification;
    for (const pair of index.calls.values()) {
        if (!isEvidenceOutput(pair.name, pair.text) || !looksLikeVerificationRun(pair.text))
            continue;
        if (verification === undefined || pair.resultSeq > verification.seq)
            verification = { seq: pair.resultSeq, name: pair.name, ok: !verificationFailed(pair) };
    }
    return {
        todosComplete,
...
function trailingSummary(index, call, maxChars) {
    ...
    for (const other of index.calls.values()) {
        if (other.resultSeq <= call.resultSeq)
            continue;
        total += 1;
...
function verificationEvidence(index, newest, budget) {
    ...
    if (newest !== undefined && looksLikeVerificationRun(newest.text))
        return { text: '', call: undefined };
    let run;
    for (const pair of index.calls.values()) {
        if (!isEvidenceOutput(pair.name, pair.text) || !looksLikeVerificationRun(pair.text))
            continue;
        if (run === undefined || pair.resultSeq > run.resultSeq)
            run = pair;
    }
    if (run === undefined)
        return { text: '', call: undefined };
 ...
 */
export function inspectRecoverySignal(events, maxItemChars) {
    const index = buildEvidenceIndex(events);
    if (!index)
        return undefined;
    ...
    if (runs.some(run => !verificationFailed(run)))
        return undefined;
...
function checkpointEvidence(index, seq, budget, current = false) {
    ...
    let latest;
    for (const pair of index.calls.values()) {
        if (!isEvidenceOutput(pair.name, pair.text))
            continue;
 ...
 */
export function itemBudget(count, maxItemChars, maxInputChars) {
...
export function analyzeStructuredRoute(events, maxCandidates = 8, maxItemChars = 20_000, maxInputChars = 60_000, options = {}) {
    const index = buildEvidenceIndex(events);
    if (!index)
        return undefined;
    ...
    for (const group of groups) {
        ...
        if (candidates.length >= 3) {
            if (reviewed.select.has(key))
                continue;
            const decision = { kind: 'select', source: 'structured', confidence: 1, reason: 'trusted workflow candidate envelope', fingerprint: stableHash({ kind: 'select', candidates, scope: scopeKey }), ...(group.scope === undefined ? {} : { scope: group.scope }), candidates };
            if (options.processed?.(decision.fingerprint))
                continue;
            return decision;
        }
        if (reviewed.compare.has(key))
            continue;
        const decision = { kind: 'compare', source: 'structured', confidence: 1, reason: 'trusted workflow candidate envelope', fingerprint: stableHash({ kind: 'compare', candidates, scope: scopeKey }), ...(group.scope === undefined ? {} : { scope: group.scope }), candidates: [candidates[0], candidates[1]] };
        if (options.processed?.(decision.fingerprint))
            continue;
        return decision;
    }
 ...
 */
export function buildSemanticRouteView(problem, events, maxCandidates, maxItemChars = 20_000, maxInputChars = 60_000) {
    const index = buildEvidenceIndex(events);
    if (!index)
    ...
    for (const [callId, pair] of [...index.calls.entries()].reverse()) {
        ...
        if (!pair.ok || !isEvidenceOutput(pair.name, pair.text))
            continue;
    ...
    const entries = [
        ...rawArtifacts.map(artifact => ({ kind: 'artifact', callId: artifact.callId, tool: artifact.tool, callSeq: artifact.callSeq, content: artifact.text })),
        ...rawCheckpoints.map(checkpoint => ({ kind: 'checkpoint', seq: checkpoint.seq, content: semanticTodoBody(checkpoint.todos, maxItemChars) })),
    ];
    ...
    if (taskOverhead >= maxInputChars)
        taskText = '';
    else if (taskOverhead + taskText.length > maxInputChars)
        taskText = sanitizeVerifierText(taskText, maxInputChars - taskOverhead);
 ...
 */
export function semanticReferencesVisible(output, visible) {
    if (output.kind === 'track')
        return output.checkpointSeqs.every(seq => visible.checkpointSeqs.has(seq));
    if (output.kind === 'none')
        return true;
    return output.candidateCallIds.every(callId => visible.candidateCallIds.has(callId));
}
export function semanticDecision(output, events, maxItemChars = 20_000, maxInputChars = 60_000, visible) {
    if (output.kind === 'none')
        return undefined;
    const index = buildEvidenceIndex(events);
    if (!index)
        return undefined;
    if (visible !== undefined && !semanticReferencesVisible(output, visible))
        return undefined;
    if (output.kind === 'track') {
    ...
    const candidates = output.candidateCallIds.map((callId, i) => {
        ...
        if (!pair || !pair.ok || !isEvidenceOutput(pair.name, pair.text))
            return undefined;
        return { id: callId, groupId: 'semantic', label: pair.name + ' ' + (i + 1), content: sanitizeVerifierText(pair.text, perItem), identity: sanitizeVerifierText(pair.text, 1_000_000_000), callId, fromSeq: pair.callSeq, toSeq: pair.resultSeq, reviewStage: 'artifact' };
    ...
    if (output.kind === 'compare' ? reviews.compare.has(key) : reviews.select.has(key))
        return undefined;
 ...
 */
export function routedRepeats(decision, configured, trackRepeats = configured) {
    if (decision.kind === 'track')
        return Math.max(1, trackRepeats);
...
export class AutoVerifierRouter {
    ...
    observe(report) {
        ...
        catch { /* an advisory UI observer must never change routing */ }
    }
    state(agent) {
        ...
        if (state.taskStartSeq !== taskStartSeq) {
            ...
            state.deliveryConsumed = undefined;
    ...
    reserve(agent, phase, fingerprint, expectedCalls, policy) {
        ...
        if (!state || state.inFlight || state.completed.has(fingerprint))
            return undefined;
        ...
        if (final) {
            if (state.sessionFinalAttempts >= policy.maxFinalPerSession)
                return undefined;
        }
        ...
        if (state.taskModelCalls + expectedCalls + floor > policy.maxModelCallsPerTask)
            return undefined;
        if (state.sessionModelCalls + expectedCalls + floor > policy.maxModelCallsPerSession)
            return undefined;
        const reservation = { id: this.instance + '-' + (++this.serial), phase, fingerprint, taskStartSeq: state.taskStartSeq, expectedCalls, attempt: attempts + 1 };
    ...
    promote(agent, reservation, phase, fingerprint, expectedCalls, policy) {
        ...
        if (reservation.phase !== 'semantic' || (phase !== 'compare' && phase !== 'select' && phase !== 'track'))
            return false;
        if (state.completed.has(fingerprint))
            return false;
     ...
     */
    processAttemptCount(agent) { return this.state(agent)?.processAttempts ?? 0; }
    /** Whether this task already bought at least one process-selection cycle. */
    hasProcessAttempt(agent) { return this.processAttemptCount(agent) > 0; }
     ...
     */
    preferFinal(agent) { const state = this.state(agent); if (state)
        state.finalPreferred = true; }
    /**
    ...
    finalPreferred(agent) { return this.state(agent)?.finalPreferred ?? false; }
    /**
     ...
     */
    deliveryConsumed(agent, signature) { return this.state(agent)?.deliveryConsumed === signature; }
    /** Mark the delivery-phase signal as spent, so the same finished state stops skipping routing. */
    consumeDelivery(agent, signature) { const state = this.state(agent); if (state)
        state.deliveryConsumed = signature; }
