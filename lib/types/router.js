...
export class AutoVerifierRouter {
    ...
    reserve(agent, phase, fingerprint, expectedCalls, policy) {
        ...
        if (attempts >= maxTask || sessionAttempts >= maxSession)
            return undefined;
        ...
        if (state.sessionModelCalls + expectedCalls + floor > policy.maxModelCallsPerSession)
            return undefined;
        const reservation = { id: this.instance + '-' + (++this.serial), phase, fingerprint, taskStartSeq: state.taskStartSeq, expectedCalls, attempt: (final ? state.finalAttempts : state.routeAttempts) + 1 };
        ...
        if (final) {
            ...
            state.sessionProcessAttempts += 1;
        }
        else {
