...
export function semanticDecision(output, events, maxItemChars = 20_000, maxInputChars = 60_000) {
    ...
    const candidates = output.candidateCallIds.map((callId, i) => {
        ...
        if (!pair || !isEvidenceOutput(pair.name, pair.text))
            return undefined;
...
export class AutoVerifierRouter {
    ...
    reserve(agent, phase, fingerprint, expectedCalls, policy) {
        ...
        if (attempts >= maxTask || sessionAttempts >= maxSession || state.taskModelCalls + expectedCalls > policy.maxModelCallsPerTask || state.sessionModelCalls + expectedCalls > policy.maxModelCallsPerSession)
            return undefined;
