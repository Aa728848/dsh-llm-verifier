    ...
    for (const pair of index.calls.values()) {
        ...
        if (pair.name === 'verifier_select' && Array.isArray(row.candidates) && row.candidates.every(value => typeof value === 'string')) {
            ...
            if (contents.length >= 3)
                reviews.select.add(candidateSetKey(contents));
            else if (contents.length === 2)
                reviews.compare.add(candidateSetKey(contents));
        }
...
export function semanticDecision(output, events, maxItemChars = 20_000, maxInputChars = 60_000, visible) {
    ...
    if (output.kind === 'compare' ? reviews.compare.has(candidateSetKey(contents)) : reviews.select.has(candidateSetKey(contents)))
        return undefined;
