    ...
    for (let attempt = 0; attempt < 64; attempt += 1) {
        ...
        if (kept.length === 1) {
            ...
            if (next < current.content.length) {
                kept = [{ ...current, content: sanitizeVerifierText(current.content, next) }];
                continue;
            }
            kept = [];
            omitted += 1;
        ...
        if (nextTask < taskText.length) {
            taskText = sanitizeVerifierText(taskText, nextTask);
            continue;
        }
        taskText = '';
    }
