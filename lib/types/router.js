...
export class AutoVerifierRouter {
    ...
    reserve(agent, phase, fingerprint, expectedCalls, policy) {
        ...
        if (state.sessionModelCalls + expectedCalls + floor > policy.maxModelCallsPerSession)
            return undefined;
        const reservation = { id: String(++this.serial), phase, fingerprint, taskStartSeq: state.taskStartSeq, expectedCalls, attempt: (final ? state.finalAttempts : state.routeAttempts) + 1 };
