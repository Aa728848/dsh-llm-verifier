...
export function unwrapVolatileConfig(value) {
    if (value && typeof value === 'object' && 'get' in value && typeof value.get === 'function') {
        return unwrapVolatileConfig(value.get());
    }
    return value;
}
/**
...
const volatileRefSchemas = new WeakSet();
/** Whether a value implements the shared cosmokit volatile reference protocol. */
export function isVolatileConfigRef(value) {
 ...
 */
function freezeVolatileSnapshot(value, ancestors = new Set()) {
    ...
    try {
        if (Array.isArray(value))
            return Object.freeze(value.map(item => freezeVolatileSnapshot(item, ancestors)));
 ...
 */
export function createVolatileConfigRef(value) {
 ...
 */
export function installVolatileRefProtocol(schema) {
    if (volatileRefSchemas.has(schema))
        return schema;
    ...
    const shadow = {
        ...
        validate(value) {
            ...
            if (!result || typeof result !== 'object' || 'issues' in result)
                return result;
            if (isVolatileConfigRef(result.value))
                return result;
...
export function markVolatile(schema) {
    ...
    else if (typeof schema.volatile === 'function') {
        marked = schema.volatile();
    }
    else {
