 ...
 */
export class TopLogprobsRouteError extends TopLogprobsUnsupportedError {
    status;
...
export async function callTopLogprobs(route, model, prompt, maxTokens, reasoningEffort, signal, images, attempt = 1, temperature = 0.2) {
...
async function replaceFile(temporary, target) {
    for (let attempt = 1;; attempt += 1) {
        try {
            ...
            if (attempt >= 5 || code === undefined || !TRANSIENT_REPLACE_CODES.has(code))
                throw error;
...
export class TopLogprobCapabilityCache {
    ...
    isUnsupported(provider, model) {
        ...
        if (markedAt === undefined)
            return false;
        if (this.now() - markedAt > CAPABILITY_TTL_MS) {
