...
export function summarizeVerdict(toolName, value, phase, thresholds) {
    ...
    if (toolName === 'verifier_select') {
        if (row.identical === true)
            return { phase, outcome: 'identical-candidates' };
...
const ROUTE_TRIGGERS = new Set(['turn-stopping', 'plan', 'team', 'pre-step']);
const ROUTE_STAGES = new Set(['classification', 'execution', 'final', 'skipped']);
/**
...
function cleanRoute(input) {
    ...
    const counts = ['attempt', 'reservedCalls', 'evidenceKept', 'evidenceOmitted', 'evidenceChars'];
