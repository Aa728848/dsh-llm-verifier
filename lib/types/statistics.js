...
function cleanRoute(input) {
    ...
    if (input.alternativeAugmented === true)
        route.alternativeAugmented = true;
    if (typeof input.alternativeModel === 'string' && input.alternativeModel)
        route.alternativeModel = input.alternativeModel.slice(0, 120);
    return route;
}
...
export class StatisticsStore {
    ...
    async amend(cycleId, patch) {
        ...
        if (patch.replayed !== undefined && patch.replayed !== 'original' && patch.replayed !== 'candidate' && patch.replayed !== 'none')
            return false;
        let found = false;
        const operation = async () => {
            await this.load();
            ...
            if (record === undefined)
                return;
            const mergedRoute = cleanRoute({ ...record.route, ...(patch.replayed === undefined ? {} : { replayed: patch.replayed }) });
            if (mergedRoute !== undefined)
                record.route = mergedRoute;
            if (patch.outcome !== undefined) {
                const mergedVerdict = cleanVerdict({ ...(record.verdict ?? {}), outcome: patch.outcome });
                if (mergedVerdict !== undefined)
                    record.verdict = mergedVerdict;
            }
            found = true;
            await this.persist();
        };
