...
async function callExplicitTag(config, prompt, signal, images, attempt) {
    ...
    for (const image of images ?? []) {
        ...
        if (pending === undefined) {
            pending = config.attachments.saveImage({ data: image.data, mediaType: image.mediaType });
...
async function callAutomatic(config, prompt, signal, images, attempt) {
    ...
    if (!config.topLogprobCapabilities.isUnsupported(config.provider, config.model)) {
        ...
        if (route !== undefined) {
            try {
                return await callTopLogprobs(route, config.model, prompt, config.maxTokens, config.reasoningEffort, signal, images, attempt);
            }
            catch (error) {
