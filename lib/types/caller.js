 ...
 */
export function fallbackDeepFreeze(value) {
...
export function requestAttempts(error) {
...
export function partialUsage(error) {
    if (typeof error !== 'object' || error === null)
        return undefined;
...
export function attachUsage(error, usage) {
    if (typeof error === 'object' && error !== null)
...
async function retrying(config, signal, run) {
    ...
    while (true) {
        if (signal?.aborted)
            cancel(signal.reason, undefined, false);
        attempt += 1;
        ...
        try {
            ...
            if (signal?.aborted)
                cancel(signal.reason ?? error, billed, unknownFailure || billed === undefined);
...
async function callTextCompletion(config, prompt, signal, images, attempt, tolerateTruncation) {
    ...
    for (const image of images ?? []) {
        ...
        content.push({ type: 'image', attachment: await pending });
    }
    ...
    if (assembler.finish.kind === 'max-tokens') {
        if (!tolerateTruncation)
            throw unusable('model call failed: ' + failureMessage(assembler.finish), attempt, assembler.usage);
        truncated = true;
    }
    else {
        const failed = failureMessage(assembler.finish);
        if (failed !== undefined)
            throw unusable('model call failed: ' + failed, attempt, assembler.usage);
    }
    const text = assembler.blocks().filter((block) => block.type === 'text').map(block => block.text).join('');
    if (!text.trim())
        throw unusable('selected DSH model produced no text', attempt, assembler.usage);
...
export class RequestLimiter {
    ...
    constructor(limit) {
        ...
        if (!Number.isSafeInteger(limit) || limit < 1)
            throw new Error('llm-verifier: request concurrency limit must be a positive integer');
    }
...
export async function predictScoringChannel(config) {
 ...
 */
export function generationClient(base, target, maxTokens = GENERATION_MAX_TOKENS) {
    return {
        ctx: base.ctx,
        llm: base.llm,
 ...
 */
export async function generateCandidate(base, target, prompt, signal) {
...
export async function callVerifierText(config, prompt, signal) {
