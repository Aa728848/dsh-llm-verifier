...
import { analyzeAutoTask, sessionAccepted } from "./auto.js";
import { analyzeStructuredRoute, inspectDeliveryPhase, latestDirectUserSeq, semanticRouteHint } from "./router.js";
...
export function parseRouteObservation(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value))
        return undefined;
    const row = value;
    if (typeof row.cycleId !== 'string' || !row.cycleId || typeof row.trigger !== 'string' || typeof row.stage !== 'string' || typeof row.destination !== 'string')
        return undefined;
    const observation = { cycleId: row.cycleId, trigger: row.trigger, stage: row.stage, destination: row.destination };
    for (const key of ['attempt', 'reservedCalls', 'evidenceKept', 'evidenceOmitted', 'evidenceChars']) {
        const candidate = row[key];
        if (typeof candidate === 'number' && Number.isFinite(candidate) && candidate >= 0)
            observation[key] = Math.trunc(candidate);
    }
    if (typeof row.skipReason === 'string' && row.skipReason)
        observation.skipReason = row.skipReason;
    ...
    if (row.usageIncomplete === true)
        observation.usageIncomplete = true;
    if (row.canceled === true)
        observation.canceled = true;
    if (typeof row.replayed === 'string' && row.replayed)
        observation.replayed = row.replayed;
    if (row.sameCandidate === true)
        observation.sameCandidate = true;
    if (row.alternativeAugmented === true)
        observation.alternativeAugmented = true;
    if (typeof row.alternativeModel === 'string' && row.alternativeModel)
        observation.alternativeModel = row.alternativeModel;
    return observation;
}
/**
...
export function replayDecisionScores(calls) {
    return calls.map(call => {
        ...
        if (!tagged)
            return { ...head, mode: 'not-scored' };
        if (reparsed === undefined)
            return { ...head, mode: 'unreadable' };
        return { ...head, reparsed, mode: call.score !== undefined && Math.abs(call.score - reparsed) < 1e-9 ? 'match' : 'drift' };
    });
}
/**
 ...
 */
export function summarizeRouteCycles(invocations) {
    const routed = invocations.filter(record => record.route !== undefined);
    const summary = { cycles: 0, classificationRows: 0, executionRows: 0, finalRows: 0, skippedRows: 0, classificationOnly: 0, canceled: 0, usageIncomplete: 0, preStepExecutions: 0, reservedCalls: 0, actualScoringCalls: 0, preStepShare: 0, classificationOnlyShare: 0, byTrigger: {}, byDestination: {}, bySkipReason: {} };
    ...
    for (const record of routed) {
        ...
        if (route.skipReason !== undefined)
            summary.bySkipReason[route.skipReason] = (summary.bySkipReason[route.skipReason] ?? 0) + 1;
        ...
        if (route.canceled === true)
            summary.canceled += 1;
        if (route.usageIncomplete === true)
            summary.usageIncomplete += 1;
        if (route.skipReason === 'classification-only-budget')
            summary.classificationOnly += 1;
        if (route.stage === 'classification')
            summary.classificationRows += 1;
        else if (route.stage === 'execution') {
            summary.executionRows += 1;
            summary.actualScoringCalls += record.calls;
            if (route.trigger === 'pre-step')
                summary.preStepExecutions += 1;
        }
        else if (route.stage === 'final') {
            summary.finalRows += 1;
            summary.actualScoringCalls += record.calls;
        }
        else if (route.stage === 'skipped')
 ...
 */
export function summarizeProcessCycles(invocations) {
    const rows = invocations.filter(record => record.route !== undefined && record.route.destination === 'process');
    const summary = { purchased: 0, skipped: 0, byOutcome: {}, bySkipReason: {}, replayedOriginal: 0, replayedCandidate: 0, replayedNone: 0, effectiveReplacementRate: 0, sameCandidate: 0, sameCandidateRate: 0, augmented: 0, augmentedReplacementRate: 0, plainReplacementRate: 0, addedCalls: 0, generatedCalls: 0, judgeCalls: 0 };
    ...
    for (const record of rows) {
        ...
        if (route.replayed === 'candidate')
            summary.replayedCandidate += 1;
        else if (route.replayed === 'none')
            summary.replayedNone += 1;
        else
            summary.replayedOriginal += 1;
        if (route.sameCandidate === true)
            summary.sameCandidate += 1;
...
export const EVALUATION_CATEGORIES = ["code", "research", "writing", "candidates", "long-task", "conversational"];
const DEFAULT_EVALUATION_POLICY = { mode: "smart", minToolCalls: 3, maxPerTask: 2, maxPerSession: 8, threshold: 0.65 };
/**
 ...
 */
export function evaluateSample(sample, policy = DEFAULT_EVALUATION_POLICY) {
    ...
    const outcome = sample.shouldReview
 ...
 */
export function summarizeEvaluation(samples, policy = DEFAULT_EVALUATION_POLICY) {
...
export function parseEvaluationSample(value) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        return undefined;
    const row = value;
    if (typeof row.id !== "string" || !row.id || !EVALUATION_CATEGORIES.includes(row.category) || typeof row.shouldReview !== "boolean" || !Array.isArray(row.events))
        return undefined;
    const expected = Array.isArray(row.expectedPhases)
        ? row.expectedPhases.filter((phase) => phase === "compare" || phase === "select" || phase === "track" || phase === "final")
        : undefined;
    return { id: row.id, category: row.category, events: row.events, shouldReview: row.shouldReview, ...(expected === undefined ? {} : { expectedPhases: expected }) };
}
