...
import { analyzeAutoTask, automaticFeedback, compareRouteFeedbackDetail, failedAcceptanceCriteria, inspectUserInteractionPause, isSubagentSession, planModeActive, selectRouteFeedbackDetail, sessionAccepted, MAX_ROUTE_FEEDBACK_CHARS } from "./auto.js";
...
import { VerifierActivities, createActivityObserver } from "./verifier-activity.js";
...
import { cancelledCall, denyWithInfo } from "./tool-decision.js";
...
import { PriceResolver, costUsd } from "./pricing.js";
import { probeWorkspaceChanges, renderWorkspaceChanges } from "./workspace.js";
 ...
 */
export async function hasLiveActiveSubagents(ctx, agent, signal) {
    try {
...
export function apply(ctx, config = {}) {
    ...
    const record = async (toolName, agent, operation, phase = 'explicit', observation) => {
        ...
        try {
            ...
            if (calls.length > 0)
                await topicEntry.decisions.record({ ...(invocation === undefined ? {} : { id: invocation.id }), toolName, phase, startedAt, provider: selected.provider, model: selected.model, calls: boundDecisionCalls([...calls].sort((a, b) => (a.label < b.label ? -1 : a.label > b.label ? 1 : 0))) }).catch(() => { });
            return value;
        }
        catch (error) {
    ...
    const workspaceEvidence = async (agent, extracted, signal) => {
        const selected = current();
        if (!selected.autoWorkspaceEvidence)
            return undefined;
    ...
    const classifyRoute = async (agent, prompt, signal, phase, observation) => record('verifier_route_classify', agent, async (trace) => {
        ...
        const stats = { ...completion.usage, cacheHits: 0, cacheMisses: 0, estimatedCostUsd: costUsd(completion.usage, prices), topLogprobScores: 0, explicitTagScores: 1 };
    ...
    const routeFeedback = (decision, detail) => createUserMessage({ content: [{ type: 'text', text: sanitizeVerifierText('[Automatic verifier routing: ' + decision.kind + ']\n' + detail + '\nUse this independent result to continue the actual task. Do not merely restate the ranking or progress score; implement, correct, and verify the required work.', MAX_ROUTE_FEEDBACK_CHARS) }], source: { kind: 'llm-verifier', form: 'notice', summary: 'Automatic verifier routed ' + decision.kind } });
    /**
    ...
    const registerProcessIntent = async (agent, signal) => {
        ...
        if (!selected.enabled || mode === 'off' || selected.autoVerifyMode !== 'smart')
            return;
        if (signal.aborted)
            return;
        ...
        if (mode === 'recovery' && (boughtCycles > 0 || lookup.purchased))
            return;
        // Every-step: no signal required, but the per-task allowance is a hard ceiling.
        if (mode === 'every-step' && boughtCycles >= selected.maxProcessCyclesPerTask)
            return;
        ...
        if (mode === 'recovery' && recovery === undefined)
            return;
    ...
    const handleProcessQuery = async (payload) => {
        ...
        if (typeof sessionId !== 'string' || sessionId === '')
            return rpcFailure('a sessionId is required');
        ...
        try {
            legacyRpc.rpc.handle('/llm-verifier', async (endpoint, payload) => {
                ...
                if (endpoint === 'probe')
                    return handleProbe();
                if (endpoint === 'process')
                    return handleProcessQuery(payload);
    ...
    ctx.on('tools/pre-execute', async (exec, next) => {
        ...
        if (!selected.enabled || selected.autoVerifyMode === 'manual' || !selected.autoVerifyPlanMode)
            return next();
        if (exec.name !== 'exit_plan_mode' || exec.agent === undefined)
            return next();
        ...
        try {
            ...
            return denyWithInfo('[Automatic Verifier Plan Pre-review] scored ' + (verdict.score * 100).toFixed(1) + '% against a ' + (selected.autoVerifyThreshold * 100).toFixed(0) + '% threshold.\n' + findings + '\nRevise the plan to address these findings, then call exit_plan_mode again.', { name: 'VerifierPlanPreReviewDenied', code: 'VERIFIER_PLAN_PRE_REVIEW_DENIED', reason: findings });
        }
        catch (error) {
            autoRouter.fail(agent, reservation, false);
    ...
    ctx.on('agent/pre-step', async (payload, next) => {
        ...
        // approved yet.
        if (planModeActive(sessionEvents(payload.agent.session)))
            return decision;
        ...
        if (inspectUserInteractionPause(sessionEvents(payload.agent.session), latestDirectUserSeq(sessionEvents(payload.agent.session)) ?? 0, payload.turn) !== undefined)
            return decision;
        ...
        if (decision) {
            ...
            if (reservation) {
                try {
                    ...
                    if (completed && !subagentsPending && !userInteractionPending)
