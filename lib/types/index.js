            ...
            if (reservation) {
                try {
                    ...
                    if (parsed.confidence < selected.autoRouteMinConfidence) {
                        // A low-confidence or "none" classification used to disappear: the dashboard showed
                        // a task that was never routed and gave no reason. Record the decision so "not
                        // executed" is explainable.
                        await recordSkippedRoute(agent, 'none', 'semantic', parsed.kind === 'none' ? 'none' : 'low-confidence');
                    }
                    if (parsed.confidence >= selected.autoRouteMinConfidence) {
                        if (parsed.kind !== 'none' && !semanticReferencesVisible(parsed, view)) {
                            ...
                            if (selected.autoVerifyMode === 'strict') {
                                // Strict mode refuses to treat an out-of-view citation as a silent pass.
                                // The reservation is already spent, so this retry is bounded by the route
                                // budget rather than looping on the same input.
