...
import { RequestLimiter, addUsage, attachUsage, callVerifier, callVerifierText, generateCandidate, requestAttempts } from "./caller.js";
...
import { AutoVerifierRouter, analyzeStructuredRoute, boundDecision, buildSemanticRouteView, estimateRoutedCalls, inspectDeliveryPhase, latestDirectUserSeq, nextDiagnosticCycleId, parseSemanticRoute, routedRepeats, semanticDecision, semanticReferencesVisible, semanticRouteHint } from "./router.js";
    ...
    const bestOfN = async (agent, task, count, repeats, signal, criteriaInput) => record('verifier_best_of_n', agent, async (trace) => {
        ...
        const attempts = await Promise.all(Array.from({ length: count }, async (_unused, index) => {
            ...
            try {
                ...
                return { ok: false, message: error instanceof Error ? error.message : String(error), billed: partialStats(error) };
            }
        }));
        const survivors = [];
        const failures = [];
        ...
        try {
            ...
            if (bounded.kind === 'compare') {
                ...
                if (!autoRouter.commit(agent, reservation, admittedLastSeq))
                    return undefined;
                return routeFeedback(bounded, compareRouteFeedbackDetail([candidateRef(bounded.candidates[0]), candidateRef(bounded.candidates[1])], { winner: result.winner, scoreA: result.scoreA, scoreB: result.scoreB, ...(result.identical === true ? { identical: true } : {}) }));
            }
            ...
            if (!autoRouter.commit(agent, reservation, admittedLastSeq))
                return undefined;
            return routeFeedback(bounded, selectRouteFeedbackDetail(bounded.candidates.map(candidateRef), { index: result.index, ranking: result.ranking, scores: result.scores, ...(result.identical === true ? { identical: true } : {}) }));
        }
        catch (error) {
            autoRouter.fail(agent, reservation, false);
            ...
            if (reservation) {
                try {
                    ...
                    if (decision.kind === 'compare') {
                        ...
                        agent.steer(routeFeedback(decision, compareRouteFeedbackDetail([candidateRef(decision.candidates[0]), candidateRef(decision.candidates[1])], { winner: result.winner, scoreA: result.scoreA, scoreB: result.scoreB, ...(result.identical === true ? { identical: true } : {}) })));
                        return;
                    }
                    if (decision.kind === 'select') {
                        ...
                        agent.steer(routeFeedback(decision, selectRouteFeedbackDetail(decision.candidates.map(candidateRef), { index: result.index, ranking: result.ranking, scores: result.scores, ...(result.identical === true ? { identical: true } : {}) })));
                        return;
                    }
