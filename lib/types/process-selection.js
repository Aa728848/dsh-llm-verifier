...
import { addUsage, emptyUsage, GENERATION_TEMPERATURE } from "./caller.js";
import { stableHash } from "./cache.js";
import { PROCESS_CRITERIA } from "./core.js";
import { mergeRunStats, partialStats } from "./engine.js";
import { VerifierActivities, classifyProcessOutcome } from "./verifier-activity.js";
import { estimateRoutedCalls, itemBudget } from "./router.js";
/**
 ...
 */
export function resolveAlternativeTargets(value) {
    const targets = [];
 ...
 */
export function alternativeTargetAt(targets, index) {
    if (targets.length === 0)
        return undefined;
 ...
 */
export function resolveAlternativeTarget(value) {
 ...
 */
export function buildFailureNotice(context) {
 ...
 */
export function buildAlternativeRequest(options, signal, failureContext, target) {
...
function boundContextSection(label, body, share, sanitize) {
    if (share < 1)
        return '';
    if (label !== EVIDENCE_LABEL || body.length <= share)
        return sanitize(body, share);
    const notice = '[Earlier ' + (body.length - share) + ' characters omitted; showing the most recent evidence]\n';
    if (notice.length >= share)
        return body.slice(-share);
...
export function renderCandidateView(candidate, budget) {
    ...
    if (actionsBlock.length + overhead > budget)
        return undefined;
...
function buildProcessContext(input) {
    ...
    if (task === '')
        return { ok: false, reason: 'the task statement is empty after redaction' };
    ...
    if (evidence !== '')
        sections.push({ label: EVIDENCE_LABEL, body: evidence });
    if (constraints !== '')
        sections.push({ label: CONSTRAINTS_LABEL, body: constraints });
    if (tools !== '')
        sections.push({ label: TOOLS_LABEL, body: tools });
 ...
 * @returns The view, or the specific length reason it was refused.
 */
export function buildProcessSelectView(input) {
 ...
 */
export function renderToolDigest(tools) {
...
export class ProcessCycleStore {
    ...
    async lookup(sessionId, taskStartSeq) {
        try {
            ...
            if (error.code === 'ENOENT')
                return { ok: true, purchased: false, count: 0 };
...
export class ProcessSelector {
     ...
     */
    activity(sessionId) { return this.activities.read(sessionId, this.deps.now()); }
    ...
    clearAll() {
        ...
        for (const sessionId of [...this.cycles.keys()])
            this.abort(sessionId, 'the settings changed');
    }
    ...
    async beginCycle(options, intent, settings, startedAt) {
        ...
        if (count !== wanted)
            this.deps.logger.warn('llm-verifier process selection: ' + wanted + ' candidates were configured without a tournament seam; comparing one pair instead');
        ...
        const expected = count === 2
            ...
            : (count - 1) + estimateRoutedCalls({ kind: 'select', candidates: new Array(count).fill(null) }, PROCESS_REPEATS, criteria.length) * judges;
        ...
        const cleanup = () => {
            ...
            if (this.cycles.get(intent.sessionId) === phase)
                this.cycles.delete(intent.sessionId);
        };
    ...
    async settledUsage(cycle) {
        ...
        for (const dispatch of cycle.dispatches) {
            try {
                ...
                if (candidate.usage.usageIncomplete)
        ...
        try {
            ...
            try {
                ...
                if (partial === undefined)
                    failureUsage.usageIncomplete = true;
                else
                    mergeRunStats(failureUsage, partial);
            ...
            if (replaced) {
                ...
                for (const chunk of winner.chunks)
                    yield chunk;
                return;
            }
            for (const chunk of original)
