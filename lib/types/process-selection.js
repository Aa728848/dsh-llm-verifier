...
export function buildProcessView(input) {
    ...
    const sections = [];
    if (input.evidence !== undefined && input.evidence.trim() !== '')
        sections.push('RECENT EXECUTION EVIDENCE:\n' + input.evidence.trim());
    if (input.constraints !== undefined && input.constraints.trim() !== '')
        sections.push('REQUEST CONSTRAINTS:\n' + input.constraints.trim());
    if (input.tools !== undefined && input.tools.trim() !== '')
