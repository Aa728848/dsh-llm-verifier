 ...
 */
export function buildAlternativeRequest(options, signal, failureContext) {
    return {
...
export function buildProcessView(input) {
    ...
    if (candidateA === undefined)
        return { ok: false, reason: 'candidate A has more tool-call text than the ' + perCandidate + '-character candidate budget; replaying the original reply' };
    const candidateB = render(input.alternative);
    if (candidateB === undefined)
        return { ok: false, reason: 'candidate B has more tool-call text than the ' + perCandidate + '-character candidate budget; replaying the original reply' };
...
export class ProcessSelector {
    ...
    async *handle(options, next, intent) {
        ...
        const observation = { cycleId: reservation.id, trigger: 'llm-stream', stage: 'process', destination: 'process', attempt: reservation.attempt, reservedCalls: reservation.expectedCalls, replayed: 'original', generatedCalls: 0, judgeCalls: 0, sameCandidate: false, ...(intent.failureContext === undefined ? {} : { alternativeAugmented: true }) };
