...
export const UNPRICED = { input: 0, output: 0, cachedInput: 0, source: 'none' };
/**
...
export function priceEntry(value) {
 ...
 */
export function mergeProviderDocument(provider, document, into) {
    ...
    for (const [key, value] of Object.entries(root)) {
        ...
        if (block === undefined)
            continue;
        for (const [id, model] of Object.entries(block))
            models[id] = model;
    }
    // models.dev: a single { models: { modelId: model } } block.
    for (const [id, model] of Object.entries(object(root.models) ?? {}))
        models[id] = model;
    let added = 0;
...
export function parseModelsDevDocument(document, into) {
    let added = 0;
    for (const [provider, value] of Object.entries(object(document) ?? {}))
        added += mergeProviderDocument(provider, value, into);
    return added;
}
 ...
 */
export function resolveCatalogDataDir(from = dirname(fileURLToPath(import.meta.url)), home = process.env.DSH_HOME?.trim()) {
...
export function loadCatalogTable(dataDir) {
    const table = new Map();
    let files;
    try {
...
export class PriceResolver {
    ...
    catalogTable() {
        ...
        if (this.catalog !== undefined && now - this.catalog.at <= ttl)
            return this.catalog.table;
    ...
    async onlineTable() {
        const now = (this.deps.now ?? Date.now)();
        const ttl = this.deps.ttlMs ?? PRICE_TTL_MS;
        if (this.online !== undefined && now - this.online.at <= ttl)
            return this.online.table;
        this.onlineFlight ??= (async () => {
            ...
            try {
                ...
                if (response.ok)
                    parseModelsDevDocument(JSON.parse(await response.text()), table);
            }
