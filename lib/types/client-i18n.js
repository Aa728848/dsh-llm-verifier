...
export const zh = {
    ...
    'field.autoProcessSelection.help': '任务连续两次验证运行失败时，自动额外生成一份备选回复并由裁判挑选更优方案，辅助打破死循环。',
    ...
    'field.autoProcessAlternativeModel.help': '用于生成备选方案的指定模型（格式 provider/model）。留空则使用当前会话模型重新采样。',
...
export const en = {
    ...
    'field.autoProcessSelection.help': 'When a task fails two consecutive verification runs, generates an alternative reply and lets the judge choose the better path to break loops.',
    ...
    'field.autoProcessAlternativeModel.help': 'Alternative model (provider/model) used to generate the candidate reply. Leave empty to resample using the session model.',
 ...
 */
export function verifierActivityText(view, t) {
    const active = view?.active;
    if (active !== undefined && active !== null) {
        if (active.stage === 'route') {
            if (active.phase !== 'reviewing')
                return { tone: 'busy', text: t['activity.route.classifying'] };
            const calls = activityCount(active.expectedCalls, 1);
            if (active.destination === 'select')
                return { tone: 'busy', text: tFormat(t['activity.route.select'], { n: calls }) };
            if (active.destination === 'track')
                return { tone: 'busy', text: tFormat(t['activity.route.track'], { n: calls }) };
    ...
    if (settled?.stage === 'final') {
        if (settled.outcome === 'accepted')
            return { tone: 'ok', text: t['activity.final.accepted'] };
        if (settled.outcome === 'rejected')
            return { tone: 'error', text: t['activity.final.rejected'] };
        return null;
    }
