...
*/
function inspectRecoverySignal(events) {
	const index = buildEvidenceIndex(events);
	if (!index) return void 0;
	...
	if (runs.some((run) => run.ok)) return void 0;
...
var AutoVerifierRouter = class {
	...
	reserve(agent, phase, fingerprint, expectedCalls, policy) {
		...
		const reservation = {
			...
			expectedCalls,
			attempt: (final ? state.finalAttempts : state.routeAttempts) + 1
		};
	...
	*/
	hasProcessAttempt(agent) {
		return (this.state(agent)?.processAttempts ?? 0) > 0;
	}
	finalRequired(agent) {
...
function automaticFeedback(score, baselineScore, winner, threshold, failedCriteria = [], locator, reportedCriteria) {
...
*/
function compareRouteFeedbackDetail(candidates, result, maxChars = MAX_ROUTE_FEEDBACK_CHARS) {
	...
	return bound(["Winner: " + located[winnerIndex] + " (" + percent(winning) + " vs " + percent(losing) + ").", "Implement the winning candidate and verify the required work before concluding."].join("\n"));
}
/**
...
* @returns The feedback body (the caller wraps and bounds it).
*/
function selectRouteFeedbackDetail(candidates, result, maxChars = MAX_ROUTE_FEEDBACK_CHARS) {
	...
	return bound([
		...
		...(result.ranking.length > 0 ? [...result.ranking] : candidates.map((_, index) => index)).map((index, rank) => rank + 1 + ". " + located[index] + " (" + percent(result.scores[index] ?? 0) + ")"),
		"Proceed with " + located[best] + ", implement it, and verify the required work before concluding."
	].join("\n"));
}
//#endregion
export { ScoreCache as A, routedRepeats as C, extractSession as D, semanticRouteHint as E, resolveCacheFile as M, stableHash as N, sanitizeVerifierText as O, parseSemanticRoute as S, semanticReferencesVisible as T, estimateRoutedCalls as _, failedAcceptanceCriteria as a, latestDirectUserSeq as b, sessionAccepted as c, MAX_ROUTED_CHECKPOINTS as d, analyzeStructuredRoute as f, buildSemanticRouteView as g, buildSemanticRoutePrompt as h, compareRouteFeedbackDetail as i, SingleFlight as j, sessionEvents as k, topScoreIndices as l, buildEvidenceIndex as m, analyzeAutoTask as n, isSubagentSession as o, boundDecision as p, automaticFeedback as r, selectRouteFeedbackDetail as s, MAX_ROUTE_FEEDBACK_CHARS as t, AutoVerifierRouter as u, inspectDeliveryPhase as v, semanticDecision as w, nextDiagnosticCycleId as x, inspectRecoverySignal as y };

//# sourceMappingURL=auto-B-ltcQL7.js.map