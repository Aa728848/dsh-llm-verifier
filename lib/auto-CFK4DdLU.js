...
function automaticFeedback(score, baselineScore, winner, threshold, failedCriteria = [], locator) {
	...
	return [
		...
		...failedCriteria.length > 0 ? ["Criteria below the threshold: " + failedCriteria.map((criterion) => (criterion.name ?? criterion.id) + " " + percent(criterion.score)).join("; ") + "."] : ["The judge reported no per-criterion breakdown for this review, so there is no per-requirement locator to act on; the score and verdict above are the only evidence returned."],
...
//#endregion
export { resolveCacheFile as A, semanticReferencesVisible as C, sessionEvents as D, sanitizeVerifierText as E, ScoreCache as O, semanticDecision as S, extractSession as T, estimateRoutedCalls as _, failedAcceptanceCriteria as a, parseSemanticRoute as b, sessionAccepted as c, MAX_ROUTED_CHECKPOINTS as d, analyzeStructuredRoute as f, buildSemanticRouteView as g, buildSemanticRoutePrompt as h, compareRouteFeedbackDetail as i, stableHash as j, SingleFlight as k, topScoreIndices as l, buildEvidenceIndex as m, analyzeAutoTask as n, isSubagentSession as o, boundDecision as p, automaticFeedback as r, selectRouteFeedbackDetail as s, MAX_ROUTE_FEEDBACK_CHARS as t, AutoVerifierRouter as u, inspectDeliveryPhase as v, semanticRouteHint as w, routedRepeats as x, latestDirectUserSeq as y };

//# sourceMappingURL=auto-CFK4DdLU.js.map