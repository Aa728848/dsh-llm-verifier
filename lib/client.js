window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"field.autoProcessSelection.help": "默认关闭。开启后，仅 smart 模式、且同一任务最近两次已完成的验证运行都失败时，才为下一次主模型请求额外生成一份备选回复，用 proposal 判据比较后只回放胜者。每任务最多购买 1 个周期，受现有模型调用预算与最终验收保留额度共同限制；选优不等于验收，之后的最终验收照常执行。关闭时完全不进入该路径：不生成候选、不调用裁判、不缓冲响应。",
		...
		const en = {
			...
			"field.autoProcessSelection.help": "Off by default. When on, smart mode only: if the two most recent completed verification runs of the task BOTH failed, the next main-model request gets one alternative reply, compared with the proposal rubric, and only the winner is replayed. At most one cycle per task, bounded by the existing model-call budget and the final-acceptance floor; selection is never acceptance, so the final gate still runs afterwards. While off the path is never entered: no candidate generation, no judge calls, no response buffering.",
