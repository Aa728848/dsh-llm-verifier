window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const WORST_CASE_TASK_PER_JUDGE = 54;
		...
		function VerifierSettings({ remote }) {
			...
			const save = async () => {
				...
				try {
					...
					if (!draft.label || !draft.label.trim()) delete section.label;
					else section.label = draft.label.trim();
					const resolvedCacheDir = resolveCacheDirOnSave(draft.cacheDir, values(loaded.settings).cacheDir);
					if (!resolvedCacheDir) delete section.cacheDir;
					else section.cacheDir = resolvedCacheDir;
