...
function buildSemanticRouteView(problem, events, maxCandidates, maxItemChars = 2e4, maxInputChars = 6e4) {
	...
	let omitted = 0;
	const artifacts = [];
	const candidateCallIds = /* @__PURE__ */ new Set();
	...
	const checkpoints = [];
	const checkpointSeqs = /* @__PURE__ */ new Set();
	...
	artifacts.reverse();
	checkpoints.reverse();
	const token = evidenceNonce(...artifacts.map((artifact) => artifact.text), ...checkpoints.map((checkpoint) => checkpoint.body), sanitizeVerifierText(problem, 4e3));
	const renderedArtifacts = artifacts.map((artifact) => renderDelimitedBlock("ARTIFACT", token, "callId: " + artifact.callId + "\ntool: " + artifact.tool + "\nseq: " + artifact.callSeq + "\n" + artifact.text)).join("\n\n");
	const renderedCheckpoints = checkpoints.map((checkpoint) => renderDelimitedBlock("CHECKPOINT", token, "seq: " + checkpoint.seq + "\n" + checkpoint.body)).join("\n\n");
	return {
		prompt: [
...
function analyzeAutoTask(events, policy, sessionId) {
	...
	const completedWork = [...pairedCalls.map((event) => ({
		seq: results.get(String(event.data.callId)).seq,
		name: event.data.name
	})), ...successfulDispatches.map((entry) => ({
		seq: entry.seq,
...
//# sourceMappingURL=auto-fG6M4Bya.js.map