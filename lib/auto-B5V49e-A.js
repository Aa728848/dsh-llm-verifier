...
var AutoVerifierRouter = class {
	...
	reserve(agent, phase, fingerprint, expectedCalls, policy) {
		...
		if (attempts >= maxTask || sessionAttempts >= maxSession || state.taskModelCalls + expectedCalls > policy.maxModelCallsPerTask || state.sessionModelCalls + expectedCalls > policy.maxModelCallsPerSession) return void 0;
		const reservation = {
	...
	*/
	...
	budgetExhausted(agent, expectedCalls, policy) {
		...
		return state.routeAttempts >= policy.maxRoutePerTask || state.sessionRouteAttempts >= policy.maxRoutePerSession || state.taskModelCalls + expectedCalls > policy.maxModelCallsPerTask || state.sessionModelCalls + expectedCalls > policy.maxModelCallsPerSession;
	}
...
//#endregion
export { resolveCacheFile as C, SingleFlight as S, semanticRouteHint as _, sessionAccepted as a, sessionEvents as b, analyzeStructuredRoute as c, buildSemanticRoutePrompt as d, estimateRoutedCalls as f, semanticDecision as g, routedRepeats as h, isSubagentSession as i, boundDecision as l, parseSemanticRoute as m, automaticFeedback as n, AutoVerifierRouter as o, latestDirectUserSeq as p, failedAcceptanceCriteria as r, MAX_ROUTED_CHECKPOINTS as s, analyzeAutoTask as t, buildEvidenceIndex as u, extractSession as v, stableHash as w, ScoreCache as x, sanitizeVerifierText as y };

//# sourceMappingURL=auto-B5V49e-A.js.map